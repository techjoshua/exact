...
- `@exact/compiler`
  - Build-tool surface: `transform`, `transformSource`, `compileFile`, `compileProject`, `compileFileArtifacts`, `compileProjectArtifacts`, `createExactArtifactPlan`, `diffExactArtifactPlans`, `readExactArtifactManifestEntries`, `exactExportConditions`, `resolveExactArtifactImport`, `createExactArtifactGraph`, `createPackageExportMap`, `createClientIslandRegistryModule`, `createServerPartRegistryModule`, `preprocessPropPunning`.
...
Project artifact compilation now uses a manifest prepass, so a file that imports a client component from another eXact source file can split that imported tag out of its server artifact without a manual directive. The consuming server artifact keeps only the serialized boundary props and any server-owned child slot, while the manifest records the imported component ID from the producing file.
Imported client component aliases and namespace imports keep the producing component's exported boundary identity. For example, `import { ClientWidget as Widget }` and `<Widgets.ClientWidget />` both emit a `ClientWidget` boundary, so hydration registries can stay keyed by exported component names instead of local binding names.
