...
- `@exact/compiler`
  ...
  - CLI: `exactc`.
- `@exact/ssr`
  - Server render surface: `renderToString(vnode, options?)`, `renderToStringAsync(vnode, options?)`, `renderToStream(vnode, options?)`, `renderToDocumentStream(vnode, options?)`, `renderToHydratableDocumentStream(vnode, options?)`.
...
- Initial document rendering can stream as newline-delimited JSON through `renderToDocumentStream()` / `renderToHydratableDocumentStream()`. Document streams emit `start`, `shell`, optional root `replace` when async server tasks settle into different HTML, optional `hydration`, and `complete`.
