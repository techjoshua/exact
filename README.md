...
- Compiled `this.state` writes are conditional: primitives use identity checks, while plain JSON objects and arrays reconcile unchanged branches in place. Repeated parsed API responses therefore do not cascade reactive updates, and arrays rendered through `this.map()` retain records by their declared key across reorders.
...
- The compiler rejects setup-time state aliases captured by common async callbacks and unmanaged browser-global listeners. Read state inside the callback, use `this.task.client(...)` for browser work, or use `peek(() => value)` when an intentional snapshot is required.
