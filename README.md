...
- `@exact/hydrate` coalesces same-tick action and refresh operations into a strict `type: "batch"` endpoint request. Batch operations are validated and dispatched independently in order; malformed top-level requests still fail as a whole, while operation-level errors return ordered per-operation results.
