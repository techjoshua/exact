# Language contribution protocol

This reference defines the implemented package protocol for trusted framework plugins and
language providers. Use [language tools](language-tools.md) for host operation and editor setup.
The declarations, projections, trust boundaries, limits, and lifecycle below are protocol contracts;
illustrative package examples do not imply privileged compiler behavior.

## Contract

The versioned eXact language-extension protocol has two participation levels:
...
independent from renderer, DOM, server, and browser packages. Package graph, selector, integrity, and public-export resolution belong to
`@exactjs/package-provenance`; plugin, component-library, and language hosts consume that shared
implementation.
