...
exports are framework implementation surfaces and must not be treated as stable application
contracts.
...
`@exactjs/core/framework/component-contracts`. Explicit runtime-owned boundary artifacts live in
the separate `@exactjs/core/framework/runtime-component-artifacts` module so merely reading a
compiled contract cannot retain component constructors. Compiler-selected render-only, durable,
and direct-server constructors likewise use separate construction subpaths. The ownership release
gate checks both module classification and this symbol-level exclusion so an internal helper cannot
drift back into the root through a mixed application module.
