...
| RF11 | P1                             | Harness returns before lazy hydration and freezes count at zero              | Include initial loading in settlement and compute count afterward            |
...
Start with RF11, RF01/RF02, RF03/RF04, RF08, and the sample build repair in RF16. The
...
### RF11: Wait for lazy island hydration in the client/server test harness

**Finding: a specific harness race reproduced; compiled continuations can be tested.**
September 19, 19:10 and 20:50; September 20, 18:05. With a real generated lazy registry,
`mountClientServerTest` returns while the first island may still be loading. The first cold click
was missed. After waiting for `data-exact-client-hydrated="true"`, the same fixture performs both
continuations, rejection, and recovery correctly. A direct `testComponent(Workspace).mount()` also
resolves for this compiled client component.

[The harness](../../packages/testing/src/client-server.ts) snapshots `hydration.length` before
calling `settle()`, leaving `hydratedIslands` at zero even after `hydration` contains an observation.
[Client settlement](../../packages/hydrate/src/runtime/client.ts) tracks pending requests, not the
initial lazy island-loading promises. Thus simply moving the count after the existing `settle()`
would not completely fix initial readiness.

**Recommendation:** make initial loading/adoption part of an explicit owned readiness contract,
then have mount await it and calculate the settled count. Propagate load/adoption failures and
respect disposal/cancellation. Keep request settlement separately understandable. Do not replace the
harness or declare continuation-owning islands untestable.

**Acceptance:** a cold lazy loader cannot cause the first post-mount click to disappear; count and
observations agree; failed loading rejects meaningfully; disposing while loading cannot mount later.
Drive two continuations and a child update through the generated registry and real handler. A browser
smoke test should supplement jsdom for the completed fix, not SSR string assertions alone.
