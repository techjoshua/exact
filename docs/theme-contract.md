...
This reference defines the shipped `exact-theme/1` token, resolver, and role contract.
Use [the theme guide](theme.md) for setup and ordinary application usage. Contract formulas,
validation rules, and acceptance requirements belong here rather than in a historical proposal.

The automated browser gate currently establishes the Chromium baseline described in the guide;
the complete cross-engine, forced-colors, zoom, text-spacing, SSR, and hydration acceptance matrix
below remains a conformance target, not a claim that every combination is automated.

Normative **must**, **should**, and **may** distinguish required, recommended, and optional behavior.
Examples are illustrative unless a surrounding requirement makes them normative.
