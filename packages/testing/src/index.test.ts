...
import { createContext, createErrorContext, createVNode, ErrorContext, type Component } from "@exact/core";
...
describe("component testing", () => {
  it("reads and writes state, contexts, components, and events", async () => {
    ...
    function Child(this: Component<{}>) { const name = this.getContext(Name); return () => createVNode("span", null, name); }
  ...
  it("settles retained asynchronous component tasks", async () => {
    function AsyncPanel(this: Component<{ ready: boolean }>) {
      ...
      this.task(async () => { await Promise.resolve(); this.state.ready = true; });
  ...
  it("rejects ambiguous singular queries and installs runner-neutral matchers", async () => {
    function Buttons() { return () => createVNode("div", null, createVNode("button", null, "One"), createVNode("button", null, "Two")); }
    ...
    installExactMatchers({ extend(matchers) { installed = matchers; } });
  ...
  it("mounts intrinsic trees and supports recursive root component types", async () => {
    ...
    function Recursive(this: Component<{}>, props: { depth: number }) {
      return () => props.depth ? createVNode(Recursive, { depth: props.depth - 1 }) : createVNode("p", null, "Done");
    }
  ...
  it("cleans failed mounts and reports stale components as unmounted", async () => {
    ...
    function Missing(): never { throw new Error("construct failed"); }
    await expect(testComponent(Missing).mount()).rejects.toThrow();
    expect(document.body.children.length).toBe(before);

    function Stable() { return () => createVNode("p", null, "Stable"); }
  ...
  it("rejects managed containers and reports replaced element handles", async () => {
    ...
    function Existing(this: Component<{ alternate: boolean }>) {
      ...
      return () => this.state.alternate ? createVNode("span", null, "New") : createVNode("button", null, "Old");
    }
    const first = await testComponent(Existing).container(container).mount();
    await expect(testComponent(Existing).container(container).mount()).rejects.toThrow("already has a mounted eXact root");
  ...
  it("finalizes the view even when component teardown reports an error", async () => {
    ...
    function BrokenTeardown(this: Component<{}>) {
      this.onUnmount(() => { throw new Error("teardown failed"); });
      return () => createVNode("p", null, "Mounted");
    }
    const teardownErrors = createErrorContext();
    teardownErrors.report = () => { throw new Error("teardown failed"); };
  ...
  it("excludes inaccessible roles and handles reusable regular expressions", async () => {
    function Content() { return () => createVNode("div", null,
  ...
  it("settles asynchronous event handlers and routes their failures", async () => {
    ...
    function AsyncButton(this: Component<{ done: boolean }>) {
      ...
      return () => createVNode("button", { onClick: async () => { await gate; this.state.done = true; } }, this.state.done ? "Done" : "Run");
    }
    const view = await testComponent(AsyncButton).mount();
    let settled = false;
    const action = view.getByRole("button").click().then(() => { settled = true; });
    ...
    function RejectingButton() {
      return () => createVNode("button", { onClick: async () => { throw new Error("event failed"); } }, "Reject");
    }
