...
export function reactElementSymbol(): symbol { return target === 18 ? REACT_ELEMENT_18 : REACT_ELEMENT_19; }
...
export type ReactDispatcher = {
  ...
  useReducer(reducer: (state: unknown, action: unknown) => unknown, initialArg: unknown, initializer?: (value: unknown) => unknown): readonly [unknown, (action: unknown) => void];
  ...
  useImperativeHandle(ref: ReactRef<unknown> | undefined, create: () => unknown, deps?: DependencyList): void;
  useId(): string;
  useSyncExternalStore(subscribe: ExternalStoreSubscribe, getSnapshot: () => unknown, getServerSnapshot?: () => unknown): unknown;
  ...
  useActionState(action: (previous: unknown, payload: unknown) => unknown, initialState: unknown, permalink?: string): readonly [unknown, (payload: unknown) => void, boolean];
  useOptimistic(passthrough: unknown, reducer?: (state: unknown, action: unknown) => unknown): readonly [unknown, (action: unknown) => void];
...
export const ReactSharedInternals18 = {
  ...
  ReactDebugCurrentFrame: { getStackAddendum: () => "", setExtraStackFrame: (_frame: string | null) => {} },
  ReactCurrentActQueue: { current: null as unknown[] | null, isBatchingLegacy: false, didScheduleLegacyUpdate: false }
};
...
export function resolveDispatcher(): ReactDispatcher {
  const dispatcher = target === 18
    ? ReactSharedInternals18.ReactCurrentDispatcher.current
    : ReactSharedInternals19.H;
  if (!dispatcher) throw new Error("Invalid hook call. Hooks can only be called inside a React compatibility component.");
  return dispatcher;
}
...
function createOwnerFrame(component: ComponentInstance<any>, type: unknown, stateNode: unknown = null): ReactOwnerFrame {
  ...
  const frame: ReactOwnerFrame = { type, return: parentFrame ?? null, child: null, sibling: null, alternate: null, memoizedState: null, stateNode };
  if (parentFrame) {
...
export class HookHost {
  ...
  render(run: () => ReactNode): ReactNode {
    ...
    try {
      ...
      if (this.expectedHooks !== undefined && this.cursor !== this.expectedHooks) {
        throw new Error(`Rendered ${this.cursor} hooks, but the previous render used ${this.expectedHooks}`);
      }
  ...
  contextChanged(): boolean {
    return this.committed.some(slot => slot.kind === "context" && !Object.is(slot.value, this.readContext(slot.context)));
  }
  ...
  reducer(reducer: (state: unknown, action: unknown) => unknown, initialArg: unknown, initializer?: (value: unknown) => unknown): readonly [unknown, (action: unknown) => void] {
    const index = this.nextIndex();
    let slot = this.slot(index);
    if (!slot) {
  ...
  effect(effectKind: EffectKind, create: () => void | (() => void), deps: DependencyList | undefined): void {
    const index = this.nextIndex();
    let slot = this.slot(index);
    if (!slot) {
  ...
  optimistic(base: unknown, reducer?: (state: unknown, action: unknown) => unknown): readonly [unknown, (action: unknown) => void] {
    const index = this.nextIndex();
    let slot = this.slot(index);
    if (!slot) {
  ...
  private commitExternalStores(): void {
    for (const slot of this.committed) {
      ...
      if (typeof slot.unsubscribe !== "function") throw new TypeError("useSyncExternalStore subscribe must return an unsubscribe function");
      notify();
    }
  }
  ...
  private nextIndex(): number {
    if (!this.rendering || !this.working) throw new Error("Hooks can only be called while rendering a React compatibility component");
    return this.cursor++;
  }

  private slot(index: number): HookSlot | undefined { return this.working?.[index]; }
  private setSlot(index: number, slot: HookSlot): void { this.working![index] = slot; this.syncOwnerHooks(); }
...
function ownerHookList(slots: readonly HookSlot[]): ReactOwnerFrame["memoizedState"] {
  ...
  for (const slot of slots) {
    const hook = { memoizedState: ownerHookValue(slot), next: null as ReactOwnerFrame["memoizedState"] };
    if (!first) first = hook;
...
export function activeHookHost(): HookHost {
  if (!currentHost) throw new Error("Invalid hook call. Hooks can only be called inside a React compatibility component.");
  return currentHost;
}
...
function createExactDispatcher(host: HookHost): ReactDispatcher {
  const dispatcher: ReactDispatcher = {
    ...
    useReducer: (reducer, initialArg, initializer) => host.reducer(reducer, initialArg, initializer),
    ...
    useImperativeHandle: (ref, create, deps) => host.effect("layout", () => {
    ...
    useTransition: () => {
      ...
      const start = dispatcher.useCallback((scope: () => void | Promise<void>) => {
        ...
        try {
          const result = scope();
          if (isThenableValue(result)) void Promise.resolve(result).then(() => setPending(false), () => setPending(false));
          else queueMicrotask(() => setPending(false));
        } catch (error) { setPending(false); throw error; }
      }, []);
      return [Boolean(pending), start];
    },
    useDeferredValue: (value, initialValue) => host.deferred(value, initialValue, arguments.length > 1),
    use: usable => isReactContextValue(usable) ? host.usableContext(usable) : readCompatibleThenable(usable),
    useActionState: (action, initialState) => {
      const [state, setState] = dispatcher.useState(initialState);
      const [pending, setPending] = dispatcher.useState(false);
      const dispatch = dispatcher.useCallback((payload: unknown) => {
        setPending(true);
        let result: unknown;
        try { result = action(state, payload); }
        catch (error) { setPending(false); throw error; }
        if (isThenableValue(result)) void Promise.resolve(result).then(value => { setState(value); setPending(false); }, () => setPending(false));
        else { setState(result); setPending(false); }
      }, [action, state]);
...
function isReactContextValue(value: unknown): value is ReactContext<unknown> {
  return !!value && typeof value === "object" && (value as { $$typeof?: unknown }).$$typeof === REACT_CONTEXT_TYPE;
}

function readCompatibleThenable(value: PromiseLike<unknown>): unknown {
  const tracked = value as PromiseLike<unknown> & { status?: string; value?: unknown; reason?: unknown };
  if (tracked.status === "fulfilled") return tracked.value;
  if (tracked.status === "rejected") throw tracked.reason;
  if (tracked.status !== "pending") {
    tracked.status = "pending";
    tracked.then(result => { tracked.status = "fulfilled"; tracked.value = result; }, error => { tracked.status = "rejected"; tracked.reason = error; });
  }
  throw tracked;
}
...
export function activeReactCacheScope(): ReactCacheScope | undefined {
  ...
  try { return currentHost.exactContext(ReactCacheContext); }
...
export function adaptReactType(type: ReactComponentType<any> | Function): ComponentFunction<Record<string, unknown>, Record<string, unknown>> {
  ...
  const adapter = function ReactCompatibilityAdapter(this: Component<Record<string, unknown>>, props: Record<string, unknown>) {
    ...
    this.onRender(() => { if (mounted) host.scheduleCommit(); });
    this.onUnmount(() => {
      try { host.dispose(); }
      finally { removeOwnerFrame(exactInstance); }
    });
    return () => {
      ...
      const special = typeof type === "object" && type !== null ? type as ReactSpecialType : undefined;
      if (!refChanged && special?.$$typeof === REACT_MEMO_TYPE && previousMemoProps && previousRevision === revision && !host.contextChanged()) {
  ...
  Object.defineProperty(adapter, "name", { configurable: true, value: `ExactReact(${displayName})` });
...
function createClassAdapter(
  ...
  const adapter = function ReactClassCompatibilityAdapter(
    ...
    const publicInstance = new type(initialSnapshot.props, initialContext) as ReactClassInstance<Record<string, unknown>> & ClassLifecycles;
    const exactInstance = this as ComponentInstance<Record<string, unknown>>;
    const ownerFrame = createOwnerFrame(exactInstance, type, publicInstance);
    Object.defineProperty(publicInstance, "_reactInternals", { configurable: true, writable: true, value: ownerFrame });
    ...
    let pendingDidUpdate: { props: Record<string, unknown>; state: unknown; snapshot: unknown } | undefined;
    ...
    const mergeState = (partial: unknown, notify: boolean) => {
      ...
      if (typeof partial !== "object") throw new TypeError("React class setState updater must return an object or null");
      const previous = publicInstance.state;
      publicInstance.state = previous && typeof previous === "object"
    ...
    const updater = {
      setState: (update: object | null | ((previous: unknown, props: unknown) => object | null), callback?: () => void) => {
        const partial = typeof update === "function" ? update(publicInstance.state, publicInstance.props) : update;
    ...
    Object.defineProperty(publicInstance, REACT_CLASS_UPDATER, { configurable: true, value: updater });
    ...
    if (isErrorBoundary(publicInstance, statics)) {
      ...
      this.setContext(ErrorContext, {
        ...
        report: (error: unknown, options?: Parameters<typeof base.report>[1]) => {
          ...
          readReactRootRuntime(this)?.onCaughtError?.(report.error, { componentStack: stack, errorBoundary: publicInstance });
          return report;
        }
      });
    }
    ...
    const flushCommit = () => {
      ...
      if (update) {
        try { publicInstance.componentDidUpdate?.(update.props, update.state, update.snapshot); }
        catch (error) { routeClassLifecycleError(this, error, "componentDidUpdate"); }
      }
      const pendingCallbacks = callbacks.splice(0, callbacks.length);
      for (const callback of pendingCallbacks) {
        try { callback.call(publicInstance); }
        catch (error) { routeClassLifecycleError(this, error, "setState-callback"); }
      }
    };
    ...
    return () => {
      ...
      try {
        ...
        const derived = statics.getDerivedStateFromProps?.(nextSnapshot.props, publicInstance.state);
        ...
        if (!firstRender && !force) {
          if (publicInstance.shouldComponentUpdate) {
            shouldUpdate = publicInstance.shouldComponentUpdate(nextSnapshot.props, nextState, nextContext) !== false;
          } else if (publicInstance.isPureReactComponent) {
            shouldUpdate = !shallowEqualProps(previousProps, nextSnapshot.props) || !shallowEqualState(previousState, nextState);
          }
        }

        if (firstRender && !statics.getDerivedStateFromProps && !publicInstance.getSnapshotBeforeUpdate) {
          publicInstance.componentWillMount?.();
          publicInstance.UNSAFE_componentWillMount?.();
        } else if (!firstRender && shouldUpdate && !statics.getDerivedStateFromProps && !publicInstance.getSnapshotBeforeUpdate) {
        ...
        if (publicInstance.getChildContext) {
          ...
          if (!childContext || typeof childContext !== "object") throw new TypeError("getChildContext() must return an object");
  ...
  Object.defineProperty(adapter, "name", { configurable: true, value: `ExactReactClass(${displayName})` });
  return adapter;
}

function readReactRootRuntime(component: Component<Record<string, unknown>>): ReactRootRuntime | undefined {
  try { return component.getContext(ReactRootContext); }
  catch { return undefined; }
}
...
export function toExactNode(node: ReactNode): Child | Child[] {
  ...
  if (!isReactElement(node)) throw new TypeError(`Objects are not valid as a React child (${Object.prototype.toString.call(node)})`);
...
function reactElementToVNode(element: ReactElement): VNode {
  ...
  if (element.type === REACT_ACTIVITY_TYPE) {
    const children = elementProps.mode === "hidden" ? [] : childrenArray(elementProps.children).map(toExactNode).flat() as Child[];
    delete keyedProps.children;
    return createVNode(ExactFragment, keyedProps, ...children);
  }
  ...
  if (typeof element.type === "symbol") throw unsupportedType(element.type.description ?? String(element.type));
  ...
  if (element.ref !== null && element.ref !== undefined) keyedProps[REACT_REF_PROP] = envelopeReactRef(element.ref);
...
function invokeReactType(type: ReactComponentType<any> | Function, props: Record<string, unknown>, ref?: unknown): ReactNode {
  ...
  if (special.$$typeof === REACT_FORWARD_REF_TYPE && special.render) return special.render(props, ref ?? null);
  if (special.$$typeof === REACT_MEMO_TYPE && special.type) return invokeReactType(special.type, props, ref);
  ...
  if (special.$$typeof === REACT_PROVIDER_TYPE || special.$$typeof === REACT_CONTEXT_TYPE && "value" in props) {
  ...
  if (special.$$typeof === REACT_CONSUMER_TYPE || special.$$typeof === REACT_CONTEXT_TYPE) {
    const context = contextForSpecial(special);
    if (typeof props.children !== "function") throw new TypeError("A React context consumer requires a function child");
...
const ReactProfilerBoundary = function ReactProfilerBoundary(
  ...
  this.onRender(({ duration }) => {
    ...
    queueMicrotask(() => {
      try { (callback as (...args: unknown[]) => void)(props.id, phase, duration, duration, commitTime - duration, commitTime); }
      catch (error) { routeClassLifecycleError(this, error, "profiler"); }
    });
  });
...
const ReactSuspenseBoundary = function ReactSuspenseBoundary(
  ...
  this.onUnmount(() => { active = false; pending.clear(); });
...
function isReactPortal(value: unknown): value is import("./types.js").ReactPortal {
  return !!value && typeof value === "object" && (value as { $$typeof?: unknown }).$$typeof === REACT_PORTAL_TYPE;
}
...
function classProps(props: Record<string, unknown>): { props: Record<string, unknown>; ref: unknown } {
  const snapshot = snapshotProps(props);
  const ref = readReactRef(snapshot[REACT_REF_PROP]);
  delete snapshot[REACT_REF_PROP];
...
function readLegacyContext(component: Component<Record<string, unknown>>): Record<string, unknown> {
  try { return component.getContext(LegacyReactContext) as unknown as Record<string, unknown>; }
  catch { return {}; }
}

function isErrorBoundary(instance: ClassLifecycles, statics: ClassStatics): boolean {
  return typeof statics.getDerivedStateFromError === "function" || typeof instance.componentDidCatch === "function";
}
...
function routeClassLifecycleError(component: Component<Record<string, unknown>>, error: unknown, phase: string): void {
...
function normalizeReactHostProps(tag: string, props: Record<string, unknown>): void {
  const handlers = Object.entries(props).filter(([name, value]) => /^on[A-Z]/.test(name) && typeof value === "function");
  for (const [name, value] of handlers) {
    delete props[name];
    let normalized = name;
    if (name === "onFocus" || name === "onFocusCapture") normalized = name.replace("onFocus", "onFocusIn");
    else if (name === "onBlur" || name === "onBlurCapture") normalized = name.replace("onBlur", "onFocusOut");
    else if ((tag === "input" || tag === "textarea") && (name === "onChange" || name === "onChangeCapture")) {
      const type = String(props.type ?? "").toLowerCase();
      if (type !== "checkbox" && type !== "radio" && type !== "file") normalized = name.replace("onChange", "onInput");
    }
    const wrapped = reactEventHandler(value as (event: Event) => unknown, props);
    const existing = props[normalized];
    props[normalized] = typeof existing === "function"
  ...
  return function exactReactEvent(this: Element, event: Event): unknown {
    ...
    try { return handler.call(this, event); }
      ...
      if (this instanceof HTMLInputElement || this instanceof HTMLTextAreaElement || this instanceof HTMLSelectElement) {
        if (props.value !== undefined && "value" in this) this.value = String(unwrap(props.value) ?? "");
        if (this instanceof HTMLInputElement && props.checked !== undefined) this.checked = Boolean(unwrap(props.checked));
      }
    }
  };
}
...
type ClassLifecycles = ReactClassInstance<Record<string, unknown>> & {
  ...
  UNSAFE_componentWillUpdate?(props: Record<string, unknown>, state: unknown, context: unknown): void;
};
...
function contextForSpecial(special: ReactSpecialType): ReactContext<unknown> {
  ...
  if (!value || typeof value !== "object" || !("_exactToken" in value)) throw new TypeError("Invalid React context object");
  return value as unknown as ReactContext<unknown>;
}

function shallowEqualProps(previous: Record<string, unknown>, next: Record<string, unknown>): boolean {
  const previousKeys = Object.keys(previous);
  const nextKeys = Object.keys(next);
  return previousKeys.length === nextKeys.length && previousKeys.every(key => Object.prototype.hasOwnProperty.call(next, key) && Object.is(previous[key], next[key]));
}
...
function reactTypeName(type: unknown): string {
  ...
  if (type && typeof type === "object") {
    const special = type as ReactSpecialType;
    if (special.$$typeof === REACT_FORWARD_REF_TYPE) return `ForwardRef(${reactTypeName(special.render)})`;
    if (special.$$typeof === REACT_MEMO_TYPE) return `Memo(${reactTypeName(special.type)})`;
  }
  return String(type);
}

function unsupportedType(name: string): Error { return new Error(`Unsupported React component type ${name}`); }
