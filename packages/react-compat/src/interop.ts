...
import { activeHookHost, assignReactRef, createReactContextForExactToken, EXACT_COMPONENT_TYPE } from "./internals.js";
...
export function exactContextToken<T>(context: ReactContext<T>): ContextToken<T> {
  if (context._exactContextMode !== "value") {
    throw new TypeError("React context was not created with bridgeReactContext or defineInteropContext");
  }
 ...
 */
export function exposeExactComponent<P extends object, State extends object = Record<string, unknown>>(
