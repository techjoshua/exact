...
export function resolveReactCompatibility(
  ...
  return { target, source: configured?.source, strict: configured?.strict ?? true, aliases: reactCompatibilityAliases(target) };
}
...
export function validateReactReconcilerTarget(target: 18 | 19, manifest: ReactReconcilerManifest): void {
...
export function validateInstalledReactReconciler(target: 18 | 19, cwd = process.cwd()): void {
  try {
    const manifest = JSON.parse(readFileSync(findPackageManifest(cwd, "react-reconciler"), "utf8")) as ReactReconcilerManifest;
    validateReactReconcilerTarget(target, manifest);
  } catch (error) {
    if (error instanceof Error && error.message.startsWith("React compatibility target")) throw error;
  }
}

function rangeAllowsMajor(range: string, major: 18 | 19): boolean {
  if (!validRange(range)) {
    throw new Error(`React compatibility target ${major} cannot validate invalid react-reconciler peer range ${JSON.stringify(range)}`);
  }
...
export function isReactCompatibilitySource(id: string, code: string, resolved: ResolvedReactCompatibility | undefined): boolean {
  return jsxSourceOwnership(id, code, resolved) === "react";
}

/** Resolves explicit JSX ownership before semantic import inference. */
export function jsxSourceOwnership(id: string, code: string, resolved: ResolvedReactCompatibility | undefined): JsxSourceOwnership {
  ...
  if ((resolved?.strict ?? true) && reactPragma && exactPragma) {
    throw new Error(`Mixed React and eXact JSX import-source directives are not supported in ${id}`);
  }
...
function resolveTarget(target: ReactCompatibilityTarget, cwd: string): 18 | 19 {
  ...
  let version: string;
  try {
    const manifest = JSON.parse(readFileSync(findReactManifest(cwd), "utf8")) as { version?: unknown };
    version = String(manifest.version ?? "");
  } catch (error) {
    throw new Error(`Unable to detect React for compatibility mode from ${cwd}; set reactCompatibility.target to 18 or 19`, { cause: error });
  }
  const major = Number.parseInt(version, 10);
  if (major !== 18 && major !== 19) throw new Error(`React compatibility supports majors 18 and 19; detected ${version || "an unknown version"}`);
  return major;
}
...
function matchesReactFilter(id: string, pattern: ReactFilterPattern): boolean {
  ...
  return patterns.some(item => typeof item === "string" ? id.includes(item) : (item.lastIndex = 0, item.test(id)));
}
