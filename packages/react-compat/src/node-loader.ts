...
export interface ExactReactNodeLoader {
  ...
  load(
    ...
    nextLoad: (url: string, context: Readonly<{ format?: string | null }>) => Promise<{ format?: string | null; source?: string | ArrayBuffer | ArrayBufferView | null; [key: string]: unknown }>
  ): Promise<{ format?: string | null; source?: string | ArrayBuffer | ArrayBufferView | null; [key: string]: unknown }>;
}

type NodeLoadContext = Readonly<{ format?: string | null }>;
type NodeLoadResult = { format?: string | null; source?: string | ArrayBuffer | ArrayBufferView | null; [key: string]: unknown };
...
export function createExactReactNodeLoader(options: ExactReactNodeLoaderOptions = {}): ExactReactNodeLoader {
  ...
  const transform = (source: string, url: string): { code: string; map: unknown } | null => {
    ...
    const result = engine.transformModule({ id: url, source, format: "module", target: "server", sourceMap: true });
  ...
  return Object.freeze({
    ...
    async load(url: string, context: NodeLoadContext, nextLoad: NextLoad) {
      ...
      if (loaded.source === null || loaded.source === undefined || loaded.format && loaded.format !== "module" && loaded.format !== "commonjs") return loaded;
