...
export type ReducerSlot = { kind: "reducer"; value: unknown; reducer: (state: unknown, action: unknown) => unknown; dispatch: (action: unknown) => void };
...
export type EffectEventSlot = { kind: "effect-event"; implementation: (...args: any[]) => unknown; callback: (...args: any[]) => unknown };
export type DeferredSlot = { kind: "deferred"; value: unknown; input: unknown; scheduled: boolean };
export type OptimisticSlot = { kind: "optimistic"; base: unknown; value: unknown; reducer?: (state: unknown, action: unknown) => unknown; dispatch: (action: unknown) => void };
...
export function assertHookKind<K extends HookSlot["kind"]>(
  ...
  if (!slot || slot.kind !== kind) {
    throw new Error(`Hook order changed at slot ${index}: expected ${kind}, received ${slot?.kind ?? "none"}`);
  }
}
