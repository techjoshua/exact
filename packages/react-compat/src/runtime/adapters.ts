...
import type { ReactClassInstance, ReactClassType, ReactNode, ReactRef } from '../types.js';
...
/** Precompiled island implementation that owns one opaque React class-component value. */
export const ReactClassIslandImplementation = function ReactClassIsland(
	...
	const updater = {
		setState: (
			...
			const partial =
				typeof update === 'function' ? update(publicInstance.state, publicInstance.props) : update;
	...
	return () => {
		...
		try {
			...
			force = false;
			constructing = false;
			recordReactRendererTransition(exactInstance, renderTransition, finishTransitionRender);
			return withReactTransitionOwnership(renderTransition, () => output);
		} finally {
			restoreOwnerScope();
		}
	};
	...
	Record<string, unknown>,
	Record<string, unknown> & { component: ReactClassType<Record<string, unknown>> }
>;
