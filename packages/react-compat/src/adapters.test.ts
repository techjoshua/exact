...
function node(
	id: string,
	manifest: ReactCompatPackageNode['manifest'],
	dependencies: string[] = []
...
): ReactCompatPackageNode['manifest'] {
	return {
		name,
		version: '1.0.0',
...
describe('React compatibility adapter discovery', () => {
	it('finds adapters below the application dependency layer', () => {
		const registry = discoverReactCompatAdapters(
			graph([
		...
		expect(
			registry.replacements.get(
				replacementKey('query', '@tanstack/react-query', 'QueryClientProvider')
			)
		).toMatchObject({
			adapterPackage: '@example/exact-query',
			specifier: '@example/exact-query/provider'
		});
	});
	...
	it('rejects incompatible duplicate adapter and protocol versions', () => {
		...
		expect(() =>
			discoverReactCompatAdapters(
				graph([
					node('root', { name: 'app', version: '1.0.0' }, ['adapter']),
					node('adapter', incompatible)
				])
			)
			...
			graph([
				node(
					'root',
					{
						name: 'app',
						version: '1.0.0',
						exact: { reactCompatibility: { ignoreAdapters: ['@example/bad'] } }
					},
					['good', 'bad', 'query']
				),
	...
	it('leaves unsupported versions unmapped and rejects conflicts, missing direct markers, and private subpaths', () => {
		...
		expect(
			discoverReactCompatAdapters(
				graph([...base, node('adapter', adapter('@example/old', '>=4 <5'))])
			).replacements.size
		).toBe(0);
		expect(() =>
			discoverReactCompatAdapters(
				graph([
					...
					node('one', adapter('@example/one')),
					node('two', adapter('@example/two')),
					node('query', { name: '@tanstack/react-query', version: '5.80.0' })
				])
			)
		).toThrow(/replacement conflict/);
		const noMarker = { ...adapter('@example/unmarked'), dependencies: {} };
		expect(() => discoverReactCompatAdapters(graph([...base, node('adapter', noMarker)]))).toThrow(
			/directly depend/
		);
		const privateExport = { ...adapter('@example/private'), exports: { '.': './dist/index.js' } };
		expect(() =>
			discoverReactCompatAdapters(graph([...base, node('adapter', privateExport)]))
		).toThrow(/not a public package export/);
	});
	...
	it('selects variants independently for duplicate installed majors', () => {
		...
		expect(replacementsForImporter(fixture, registry, '/fixture/root/app.ts')[0]?.export).toBe(
			'QueryClientProvider'
		);
		expect(replacementsForImporter(fixture, registry, '/fixture/nested/view.ts')[0]?.export).toBe(
			'LegacyProvider'
		);
	});
