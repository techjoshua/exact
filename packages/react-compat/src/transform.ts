...
export function usesReactRuntimeImports(source: string, filename = 'input.tsx'): boolean {
	...
	for (const statement of sourceFile.statements) {
		if (
			...
			!reactRuntimeModules.has(statement.moduleSpecifier.text)
		)
			continue;
		...
		if (bindings && ts.isNamespaceImport(bindings)) runtimeBindings.add(bindings.name.text);
			...
			for (const element of bindings.elements)
				if (!element.isTypeOnly) runtimeBindings.add(element.name.text);
	}
...
export function transformReactJsx(
	...
	const result = ts.transpileModule(source, {
		...
		compilerOptions: {
			...
			...(classic
				? { jsxFactory: 'React.createElement', jsxFragmentFactory: 'React.Fragment' }
