...
export type ExactPluginHostMode =
  | "compiler"
  | "server"
  | "render"
  | "client"
...
export interface ExactRuntimePluginExtension {
  ...
  initializeApplication?(context: ExactPluginLifecycleContext): ExactPluginResource | void | Promise<ExactPluginResource | void>;
  initializeRequest?(context: ExactPluginLifecycleContext): ExactPluginResource | void | Promise<ExactPluginResource | void>;
...
export interface ExactPluginConfigController<T> {
  ...
  compilerConfig?(config: T, context: ExactPluginConfigContext): ExactCompilerPluginConfig | Promise<ExactCompilerPluginConfig>;
...
export function packageDirectlyDependsOnPluginApi(manifest: ExactPackageManifest): boolean {
  return dependencyRange(manifest.dependencies, exactPluginApiPackage) !== undefined
...
function readPlugin(value: unknown, label: string): ExactPluginDeclaration | undefined {
  ...
  assertOnlyKeys(record, ["schemaVersion", "protocolVersion", "configKey", "entries"], `${label}.exact.plugin`);
  ...
  const protocolVersion = requiredString(record.protocolVersion, `${label}.exact.plugin.protocolVersion`);
  const configKey = requiredString(record.configKey, `${label}.exact.plugin.configKey`);
  if (!/^[$A-Z_a-z][$\w]*$/.test(configKey)) throw new Error(`${label}.exact.plugin.configKey must be an identifier`);
  const entriesRecord = requiredRecord(record.entries, `${label}.exact.plugin.entries`);
  assertOnlyKeys(entriesRecord, ["config", "configTypes", "compiler", "server", "render", "client", "testing"], `${label}.exact.plugin.entries`);
  ...
  return Object.freeze({ schemaVersion: exactPluginSchemaVersion, protocolVersion, configKey, entries: Object.freeze(entries) });
}

function readForwarding(value: unknown, label: string): ExactPluginForwardingDeclaration | undefined {
  ...
  if (record.schemaVersion !== exactPluginForwardingSchemaVersion) {
    throw new Error(`${label}.exact.pluginForwarding.schemaVersion must be ${exactPluginForwardingSchemaVersion}`);
  }
  ...
  for (const name of Object.keys(include)) {
    if (matchesPackageSelectors(name, ignore)) throw new Error(`${label} cannot include and ignore ${name}`);
  }
  return Object.freeze({ schemaVersion: exactPluginForwardingSchemaVersion, include: Object.freeze(include), ignore });
}

function readConfiguration(value: unknown, label: string): Readonly<Record<string, ExactPluginConfigurationDeclaration>> {
  ...
  for (const [name, raw] of Object.entries(record)) {
    ...
    assertOnlyKeys(declaration, ["version", "subpath", "export"], `${label} configuration for ${name}`);
    const subpath = requiredString(declaration.subpath, `${label} configuration for ${name}.subpath`);
    assertPublicPackageSubpath(subpath, `${label} configuration for ${name}.subpath`);
    const exportName = requiredString(declaration.export, `${label} configuration for ${name}.export`);
    ...
    result[name] = Object.freeze({
      ...(declaration.version === undefined ? {} : { version: requiredString(declaration.version, `${label} configuration for ${name}.version`) }),
      subpath,
      export: exportName
    });
  }
...
export function matchesPackageSelectors(packageName: string, selectors: readonly string[]): boolean {
  return selectors.some(selector => selector.endsWith("/") ? packageName.startsWith(selector) : packageName === selector);
}
