...
describe('React compatibility adapter protocol', () => {
	it('parses inert owned-export metadata', () => {
		expect(
			readReactCompatAdapterDeclaration(validManifest)?.substitutions['@tanstack/react-query']
				?.variants[0]?.exports.QueryClientProvider
	...
	it('rejects reserved sources, arbitrary target fields, and dependency suppression', () => {
		...
		reserved.exact.reactCompatibility.substitutions.react =
			reserved.exact.reactCompatibility.substitutions['@tanstack/react-query'];
		delete reserved.exact.reactCompatibility.substitutions['@tanstack/react-query'];
		expect(() => readReactCompatAdapterDeclaration(reserved)).toThrow(
			/reserved framework package react/
		);
		const target = mutate();
		target.exact.reactCompatibility.substitutions[
			'@tanstack/react-query'
		].variants[0].exports.QueryClientProvider.package = 'elsewhere';
	...
	it('parses and validates root ignore policy', () => {
		expect(
			readReactCompatApplicationPolicy({
				exact: { reactCompatibility: { ignoreAdapters: ['@example/one'] } }
			})
		).toEqual({ ignoreAdapters: ['@example/one'] });
		expect(() =>
			readReactCompatApplicationPolicy({
				exact: { reactCompatibility: { ignoreAdapters: ['../one'] } }
			})
