...
export function limitedJsonResponse(context: ExactServerContext, status: number, body: unknown): ExactResponseLike {
...
export function matchesConfiguredEndpoint(request: ExactRequestLike, endpoint: string | undefined): boolean {
...
async function dispatchExactOperationAfterSecurity(
  ...
  if (!securityChecked) {
    ...
    if (security === "unauthorized") return reject(403, "forbidden", "rejected unauthorized exact invocation");
    if (security === "csrf") return reject(403, "forbidden", "rejected exact invocation with invalid csrf");
  }

  const handler = input.type === "action" ? context.actions?.[input.id] : context.refreshBoundaries?.[input.id];
  if (!handler) return reject(404, "not_found", "rejected exact invocation without registered handler");

  try {
    const requestContext = request.signal && request.signal !== context.signal ? { ...context, signal: request.signal } : context;
