...
export function seedKeyedCollectionMetadata(collection: unknown[], key: KeyExtractor): KeyedCollectionMetadata | undefined {
...
function rebuildMetadata(collection: unknown[], key: KeyExtractor): KeyedCollectionMetadata | undefined {
  try {
    const keys: string[] = [];
...
function rebindOwners(metadata: KeyedCollectionMetadata, collection: unknown[], keys: ReadonlySet<string>): void {
  for (let index = 0; index < metadata.keys.length; index++) {
...
function validateEnvelope(value: Record<string, unknown>): KeyedCollectionEnvelope {
  const allowed = new Set(["$exact", "version", "keys", "keyHash", "itemHashes", "itemsHash", "items"]);
  if (Object.keys(value).some(key => !allowed.has(key))
    ...
    || typeof value.keyHash !== "string" || !hashPattern.test(value.keyHash)
    || typeof value.itemsHash !== "string" || !hashPattern.test(value.itemsHash)) {
