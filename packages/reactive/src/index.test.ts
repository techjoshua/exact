...
describe("@exact/reactive", () => {
  it("routes scheduler failures and leaves the watcher retryable", () => {
    ...
    const scheduler = vi.fn(() => { throw new Error("schedule failed"); });
    watch(() => void state.count, scheduler, { onError: error => errors.push(error) });

    expect(() => { state.count++; }).not.toThrow();
    expect(() => { state.count++; }).not.toThrow();
  ...
  it("routes onSchedule failures and leaves the watcher retryable", () => {
    const state = reactive({ count: 0 });
    const errors: unknown[] = [];
    const onSchedule = vi.fn(() => { throw new Error("onSchedule failed"); });
    watch(() => void state.count, undefined, { onSchedule, onError: error => errors.push(error) });
  ...
  it("rejects recreated dynamic extractors whose captured behavior disagrees", () => {
    const state = reactive({ records: [{ id: "a", slug: "first" }] });
    const extractor = (field: "id" | "slug") => (item: unknown) => (item as { id: string; slug: string })[field];
    registerReactiveListKey(state.records, extractor("id"), "ZList");

    expect(() => registerReactiveListKey(
      state.records,
      extractor("slug"),
      "AList"
    )).toThrow("(AList and ZList)");
  });
  ...
  it("rolls back object writes and emits no notifications when a batch throws", () => {
    ...
    const observer = vi.fn(() => `${state.record.title}:${state.record.nested.count}:${"added" in state.record}`);
    watch(observer);
  ...
  it("rolls back compiler reconciliations while retaining nested proxy identity", () => {
    ...
    registerReactiveListKey(state.records, item => (item as { id: string }).id, "Records", "member:id");
    const project = state.project;
    ...
    expect(() => batch(() => {
      ...
      writeReactive(state, ["records"], [{ id: "b", title: "Changed" }, { id: "a", title: "A" }]);
      throw new Error("abort");
    })).toThrow("abort");
