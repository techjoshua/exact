...
describe("prepared plugin registry", () => {
  it("runs defaults, deepest contributors, and root mutation with undefined retention", async () => {
    ...
    const framework = createPackage(root, "@acme/framework", {
      exact: {
        pluginForwarding: {
          schemaVersion: 1,
    ...
    expect(readFileSync(path.join(root, ".exact", "plugins.d.ts"), "utf8"))
      .toContain('/// <reference types="@exact/example/config-types" />');
...
function createPackage(
  ...
  for (const [filename, contents] of Object.entries(files)) writeFileSync(path.join(location, filename), contents);
  return {
    id: name,
    location,
...
function createFixtureGraph(
  ...
  const frameworkNode: ExactPackageNode = {
    ...
    dependencies: new Map([
      ...
      ["@exact/example", { name: "@exact/example", range: "^1.0.0", kind: "dependency", targetId: plugin.id }]
    ])
  };
  return {
    rootId: rootNode.id,
