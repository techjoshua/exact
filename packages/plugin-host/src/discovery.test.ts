...
describe("plugin discovery", () => {
  it("prevents a trusted parent from laundering an untrusted forwarded child", () => {
    ...
    expect(() => discoverExactPlugins(graph, {
      mode: "trusted",
  ...
  it("traverses only explicit forwarding edges in all mode", () => {
    ...
    expect([...result.plugins.keys()].sort()).toEqual([
...
function fixtureGraph(input: {
  ...
  const rootDependencies = new Map(Object.entries(input.rootDependencies).map(([name, range]) => [
    name,
    Object.freeze({ name, range, kind: "dependency" as const, targetId: idsByName.get(name)?.[0] })
  ]));
  nodes.set("root", Object.freeze({
    id: "root",
    location: "/app",
    realPath: "/app",
    manifest: {
...
function packageFixture(name: string, version: string, manifest: Record<string, unknown>): PackageInput {
