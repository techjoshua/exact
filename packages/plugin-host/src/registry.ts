...
export async function prepareExactPluginRegistry(
  ...
  const configPath = options.configPath ? path.resolve(options.configPath) : findExactConfig(applicationRoot);
  ...
  const promise = prepareUncached({ ...options, applicationRoot, configPath, environment, hostMode });
...
export function invalidateExactPluginRegistry(applicationRoot?: string): void {
  ...
  for (const key of registryCache.keys()) if (key.includes(JSON.stringify(root).slice(1, -1))) registryCache.delete(key);
}
...
async function prepareUncached(
  ...
  const timer = setTimeout(() => controller.abort(new Error(`Plugin registry preparation exceeded ${timeoutMs}ms`)), timeoutMs);
  try {
    ...
    const resolution = await abortable(resolveExactPluginConfigurations(discovery, {
    ...
    if (controller.signal.aborted) {
      throw controller.signal.reason instanceof Error ? controller.signal.reason : new Error("Plugin registry preparation aborted");
    }
...
function createCompilerRegistry(
  ...
  for (const [name, value] of [...resolved].sort(([left], [right]) => left.localeCompare(right))) {
    ...
    const contributors = discovery.contributors
      ...
      .map(contributor => `${packageName(contributor.node)}:${contributor.declaration.subpath}#${contributor.declaration.export}`)
      .sort();
...
async function loadRuntimeExtensions(
  ...
  for (const mode of modes) {
    ...
    for (const value of [...resolved.values()].sort((left, right) => left.plugin.packageName.localeCompare(right.plugin.packageName))) {
      ...
      if (!extension || typeof extension !== "object") {
        throw new Error(`${value.plugin.packageName} ${mode} entry must export an extension or extension factory`);
      }
...
function projectionMap(
  ...
  return new Map([...resolved].flatMap(([name, value]) => value[key] === undefined ? [] : [[name, value[key]]]));
}
...
async function loadExactConfig(configPath: string | undefined): Promise<ExactConfig | undefined> {
  ...
  if (/\.[cm]?ts$/i.test(configPath)) {
    ...
    if (errors.length) {
      throw new Error(`Unable to transpile ${configPath}: ${errors.map(error => ts.flattenDiagnosticMessageText(error.messageText, "\n")).join("\n")}`);
    }
...
async function writePluginTypes(applicationRoot: string, discovery: ExactPluginDiscoveryResult): Promise<string> {
  ...
  const references = [...discovery.plugins.values()]
    ...
    .map(plugin => {
      ...
      const specifier = subpath === "." ? plugin.packageName : `${plugin.packageName}${subpath.slice(1)}`;
      return `/// <reference types="${specifier}" />`;
    });
  const contents = [
    "// Generated by @exact/plugin-host. Do not edit.",
    ...references,
    ""
  ].join("\n");
  if (!references.length) {
    if (existsSync(target)) {
      ...
      if (existing.startsWith("// Generated by @exact/plugin-host.")) await rm(target, { force: true });
    }
    return target;
  }
...
function discoverWatchFiles(
  ...
  try { files.add(findUp(applicationRoot, "package-lock.json")); } catch {}
  for (const participant of discovery.participants.values()) files.add(path.join(participant.node.location, "package.json"));
  for (const plugin of discovery.plugins.values()) files.add(path.join(plugin.node.location, "package.json"));
...
function findExactConfig(cwd: string): string | undefined {
  ...
  const names = ["exact.config.ts", "exact.config.mts", "exact.config.js", "exact.config.mjs", "exact.config.cjs"];
  while (true) {
...
function abortable<T>(promise: Promise<T>, signal: AbortSignal): Promise<T> {
  ...
  return new Promise<T>((resolve, reject) => {
    const abort = () => reject(signal.reason instanceof Error ? signal.reason : new Error("Plugin registry preparation aborted"));
