...
export async function resolveExactPluginConfigurations(
  ...
  for (const selected of [...discovery.plugins.values()].sort((left, right) => left.packageName.localeCompare(right.packageName))) {
    const entry = selected.declaration.entries.config;
    const controller = entry
      ? controllerFromModule(await importPublicPackageEntry(selected.node, entry), selected.packageName)
      : noConfigController();
    const contributors = discovery.contributors.filter(value => value.plugin === selected.packageName);
    const resolved = await resolveOnePlugin(discovery, selected, controller, contributors, options, reports);
...
async function resolveOnePlugin<T>(
  ...
  const baseContext = (): ExactPluginConfigContext => Object.freeze({
    ...
    provenance: Object.freeze({
      activationPaths: pluginParticipant?.activationPaths ?? selected.requirements.map(value => value.path),
      orderingAfter: Object.freeze([])
    })
  });
  let current = await controller.defaults(baseContext()) as T;
  if (current === undefined) throw new Error(`Plugin ${selected.packageName} defaults returned undefined`);
  ...
  for (const contributor of ordered) {
    ...
    const context: ExactPluginConfigContext = Object.freeze({
      ...
      provenance: Object.freeze({
        ...
        orderingAfter: Object.freeze(contributingDescendants(discovery, contributor.node.id, contributors)
          .map(node => packageName(node)).sort())
      })
    });
  ...
  const rootTransform = options.applicationConfig?.plugins?.[configKey as keyof NonNullable<ExactConfig["plugins"]>];
  if (rootTransform === false) {
    throw new Error(`Root configuration disables active plugin ${selected.packageName}; ignore its package instead`);
  }
  if (typeof rootTransform === "function") {
    ...
    try {
      ...
      throw new Error(`Root configuration failed for plugin ${selected.packageName}`, { cause: error });
    }
    if (result !== undefined) {
      current = result as T;
      controller.structuralValidate?.(current, context);
  ...
  if (validation !== undefined) throw new Error(`Plugin ${selected.packageName} validate() must return undefined`);
...
function orderContributors(
  ...
  while (remaining.size) {
    const ready = [...remaining].filter(id =>
      contributingDescendantIds(discovery, id, byId).every(descendant => completed.has(descendant))
    ).sort((left, right) => packageName(byId.get(left)!.node).localeCompare(packageName(byId.get(right)!.node)));
    if (!ready.length) {
      throw new Error(`Plugin configuration contributor graph contains a cycle: ${[...remaining].map(id => packageName(byId.get(id)!.node)).join(", ")}`);
    }
...
async function loadTransform<T>(contributor: ExactConfigurationContributor): Promise<ExactPluginConfigTransform<T>> {
  if (contributor.declaration.version && !validRange(contributor.declaration.version)) {
    throw new Error(`${packageName(contributor.node)} declares invalid plugin configuration version range ${contributor.declaration.version}`);
  }
  ...
  if (typeof value !== "function") {
    throw new Error(`${packageName(contributor.node)} does not export ${contributor.declaration.export} from ${contributor.declaration.subpath}`);
  }
  return value as ExactPluginConfigTransform<T>;
}

function controllerFromModule<T>(module: Record<string, unknown>, packageNameValue: string): ExactPluginConfigController<T> {
  const value = module.default ?? module.controller;
  if (!value || typeof value !== "object") {
    throw new Error(`${packageNameValue} config entry must export a default controller or named controller`);
  }
  ...
  if (typeof controller.defaults !== "function" || typeof controller.validate !== "function") {
    throw new Error(`${packageNameValue} config controller must implement defaults() and validate()`);
  }
...
function assertJsonSafe(value: unknown, label: string): void {
  ...
  const visit = (item: unknown): void => {
    ...
    seen.add(item);
    if (!Array.isArray(item) && Object.getPrototypeOf(item) !== Object.prototype) throw new Error(`${label} must be JSON-safe`);
...
function throwIfAborted(signal: AbortSignal): void {
  if (signal.aborted) throw signal.reason instanceof Error ? signal.reason : new Error("Plugin registry preparation aborted");
}
