...
export function resolveDiscoveryPolicy(config: ExactPluginDiscoveryConfig | undefined): ExactDiscoveryPolicy {
  if (!config) {
  ...
  if (config.mode === "trusted") {
    ...
    for (const packageNameValue of config.trustedPackages ?? []) {
      if (packageNameValue.endsWith("/")) throw new Error(`pluginDiscovery.trustedPackages entry ${packageNameValue} must be an exact package name`);
    }
    validateSelectors(config.trustedPrefixes ?? [], "pluginDiscovery.trustedPrefixes");
    for (const prefix of config.trustedPrefixes ?? []) {
      if (!prefix.endsWith("/")) throw new Error(`pluginDiscovery.trustedPrefixes entry ${prefix} must end in /`);
    }
    validateSelectors(config.ignore ?? [], "pluginDiscovery.ignore");
    return Object.freeze({
      mode: "trusted",
...
export function discoverExactPlugins(
  ...
  for (const dependency of root.dependencies.values()) {
    ...
    acceptParticipant(participants, edges, root, node, participation, [rootName, packageName(node)]);
    pending.push(node.id);
    if (participation.plugin) requirements.push({
  ...
  if (policy.mode !== "root") {
    ...
    while (pending.length) {
      ...
      for (const [childName, declaration] of Object.entries(forwarding.include).sort(([left], [right]) => left.localeCompare(right))) {
        const dependency = participant.node.dependencies.get(childName);
        if (!dependency) {
          throw new Error(`${manifestLabel(participant.node)} forwards ${childName} without declaring it as a dependency, optional dependency, or peer dependency`);
        }
        const ignored = matchesPackageSelectors(childName, policy.ignore)
          || matchesPackageSelectors(childName, forwarding.ignore ?? []);
        if (ignored) {
          if (declaration.required === true) {
            throw new Error(`Required forwarded plugin ${childName} is ignored through ${packageName(participant.node)}`);
          }
          continue;
        }
        if (!dependency.targetId) {
          if (dependency.kind === "optional" && declaration.required !== true) continue;
          throw new Error(`${packageName(participant.node)} requires forwarded package ${childName}, but it is not installed`);
        }
        const child = requiredNode(graph, dependency.targetId);
        if (policy.mode === "trusted" && !isTrustedChild(root, child, policy)) {
          if (declaration.required === true) {
            throw new Error(`Required forwarded package ${childName} is not trusted; add its package name or prefix to pluginDiscovery`);
          }
          continue;
        }
        const childParticipation = readExactPackageParticipation(child.manifest, manifestLabel(child));
        if (!hasParticipation(childParticipation)) {
          throw new Error(`${packageName(participant.node)} forwards ${childName}, but that package declares no eXact plugin participation metadata`);
        }
        validateParticipantMarker(child);
        const parentPaths = participant.activationPaths;
        for (const parentPath of parentPaths) {
          acceptParticipant(participants, edges, participant.node, child, childParticipation, [...parentPath, childName]);
        }
        pending.push(child.id);
        if (childParticipation.plugin) requirements.push({
          plugin: childName,
          range: dependency.range,
          required: declaration.required === true || dependency.kind !== "optional",
          parentId: participant.node.id,
          path: Object.freeze([...(parentPaths[0] ?? [rootName, packageName(participant.node)]), childName])
        });
      }
    }
  }
    ...
    for (const participant of participants.values()) {
      ...
      for (const [plugin, declaration] of Object.entries(participant.participation.configuration)) {
        ...
        if (declaration.version && !versionSatisfies(selected.version, declaration.version)) {
          throw new Error(`${packageName(participant.node)} configures ${plugin} ${declaration.version}, but selected ${selected.version}`);
        }
  ...
  return Object.freeze({
    policy,
    root,
    participants: new Map([...participants].map(([id, participant]) => [id, freezeParticipant(participant)])),
    participantEdges: new Map([...edges].map(([id, values]) => [id, new Set(values)])),
    plugins,
    contributors: Object.freeze(contributors),
    warnings: Object.freeze(policy.mode === "all"
...
function acceptParticipant(
  ...
  if (current) {
    if (!current.activationPaths.some(pathValue => pathValue.join("\0") === activationPath.join("\0"))) {
...
function selectPlugins(
  ...
  for (const [name, ranges] of grouped) {
    const candidates = [...graph.nodes.values()].filter(node => {
      ...
      return declaration !== undefined && ranges.every(requirement => versionSatisfies(packageVersion(node), requirement.range));
    });
    if (!candidates.length) {
      throw new Error(`No installed ${name} implementation satisfies ${ranges.map(value => `${value.range} via ${value.path.join(" -> ")}`).join(", ")}`);
    }
    ...
    candidates.sort((left, right) => {
      ...
      const distanceOrder = (distances.get(left.id) ?? Number.POSITIVE_INFINITY) - (distances.get(right.id) ?? Number.POSITIVE_INFINITY);
...
function validateParticipantMarker(node: ExactPackageNode): void {
  if (!packageDirectlyDependsOnPluginApi(node.manifest)) {
    throw new Error(`${packageName(node)} must directly depend on ${exactPluginApiPackage} to participate in the eXact plugin system`);
  }
  const range = dependencyRange(node.manifest.dependencies, exactPluginApiPackage)
    ?? dependencyRange(node.manifest.optionalDependencies, exactPluginApiPackage);
  if (range && !isLocalRange(range) && (!validRange(range) || !satisfies(exactPluginProtocolVersion, range))) {
    throw new Error(`${packageName(node)} declares incompatible ${exactPluginApiPackage} range ${JSON.stringify(range)}`);
  }
}

function validatePluginProtocol(name: string, declaration: ExactPluginDeclaration): void {
  if (!valid(declaration.protocolVersion) || declaration.protocolVersion.split(".")[0] !== exactPluginProtocolVersion.split(".")[0]) {
...
function validatePluginEntries(node: ExactPackageNode, declaration: ExactPluginDeclaration): void {
  for (const [kind, subpath] of Object.entries(declaration.entries)) {
    if (!manifestExportsSubpath(node.manifest.exports, subpath)) {
      throw new Error(`${packageName(node)} plugin ${kind} entry ${subpath} is not a public package export`);
    }
  }
}

function rejectAmbiguousParticipants(participants: ReadonlyMap<string, MutableParticipant>, rootId: string): void {
  ...
  for (const [name, values] of byName) {
    ...
    if (versions.length > 1 && values.some(value =>
      value.participation.forwarding || Object.keys(value.participation.configuration).length
    )) {
      throw new Error(`Participating framework package ${name} has ambiguous executable versions: ${versions.join(", ")}`);
    }
  }
}
...
function rejectConfigKeyConflicts(plugins: ReadonlyMap<string, ExactSelectedPlugin>): void {
  ...
  for (const plugin of plugins.values()) {
    ...
    if (previous) throw new Error(`Plugin configKey ${plugin.declaration.configKey} is declared by both ${previous} and ${plugin.packageName}`);
    keys.set(plugin.declaration.configKey, plugin.packageName);
  }
}

function isTrustedChild(root: ExactPackageNode, child: ExactPackageNode, policy: ExactDiscoveryPolicy): boolean {
  const name = packageName(child);
  return [...root.dependencies.values()].some(dependency => dependency.targetId === child.id)
...
function hasParticipation(value: ExactPackageParticipation): boolean {
  return value.plugin !== undefined || value.forwarding !== undefined || Object.keys(value.configuration).length > 0;
}
...
function freezeParticipant(value: MutableParticipant): ExactParticipatingPackage {
  return Object.freeze({
    ...
    activationPaths: Object.freeze(value.activationPaths.map(pathValue => Object.freeze([...pathValue])))
  });
}
...
function safePackageName(node: ExactPackageNode): string | undefined {
  return typeof node.manifest.name === "string" && node.manifest.name ? node.manifest.name : undefined;
}
...
function manifestExportsSubpath(exportsField: unknown, subpath: string): boolean {
  ...
  return Object.prototype.hasOwnProperty.call(record, subpath)
