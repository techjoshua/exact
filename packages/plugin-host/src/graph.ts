...
export function createNpmExactPackageGraph(cwd = process.cwd()): ExactPackageGraph {
  ...
  for (const [rawId, entry] of Object.entries(records)) {
    ...
    try {
      manifest = parseJson(readFileSync(manifestFile, "utf8"), manifestFile) as ExactPackageManifest;
    } catch (error) {
  ...
  for (const [id, raw] of rawNodes) {
    ...
    for (const dependency of dependencyDeclarations(raw.manifest)) {
      dependencies.set(dependency.name, Object.freeze({
        ...dependency,
...
export function createInstalledExactPackageGraph(cwd = process.cwd()): ExactPackageGraph {
  ...
  while (pending.length) {
    ...
    const manifest = parseJson(readFileSync(manifestFile, "utf8"), manifestFile) as ExactPackageManifest;
...
function freezeNode(
  ...
  let realPath: string;
  try { realPath = realpathSync.native(location); } catch { realPath = path.resolve(location); }
...
function parseJson(source: string, filename: string): unknown {
  ...
  catch (error) { throw new Error(`Unable to parse ${filename}`, { cause: error }); }
}
