...
describe("ExpressionLanguageService", () => {
  it("propagates public shape changes to diagnostic-changing consumers", () => {
    ...
    try {
      const baseline = service.synchronize([
        ...
        { kind: "upsert", filename: consumer, source: 'import { model } from "./__language_model.js";\nexport const value: number = model.value;' }
      ]);
      ...
      expect(update.affectedFiles).toEqual(expect.arrayContaining([
        model.replaceAll("\\", "/"),
        consumer.replaceAll("\\", "/")
      ]));
  ...
  it("limits implementation-only updates to the changed leaf", () => {
    ...
    try {
      ...
      const update = service.synchronize([{ kind: "upsert", filename, source: "export const value = 2;" }]);
  ...
  it("applies batched overlays and deletions without retaining stale snapshots", () => {
    ...
    try {
      service.synchronize([
        ...
        { kind: "upsert", filename: second, source: 'import { first } from "./__language_batch_first.js"; export const second = first;' }
      ]);
  ...
  it.runIf(process.platform === "win32")("case-folds identity without lowercasing reported filenames", () => {
    const service = createExpressionLanguageService({ tsconfigPath: config });
    const filename = path.join(sourceRoot, "__Language_Casing.ts");
    const alternate = `${filename[0] === filename[0]!.toUpperCase()
      ? filename[0]!.toLowerCase()
      : filename[0]!.toUpperCase()}${filename.slice(1)}`;
    const displayed = filename.replaceAll("\\", "/");
    try {
      service.synchronize([{ kind: "upsert", filename, source: "export const value: number = 1;" }]);
      const update = service.synchronize([{
        kind: "upsert",
        filename: alternate,
      ...
      expect(update.affectedFiles.filter(candidate =>
        candidate.toLowerCase() === displayed.toLowerCase()
      )).toEqual([displayed]);
      expect(update.diagnostics).toContainEqual(expect.objectContaining({
        code: "TS2322",
        filename: displayed
