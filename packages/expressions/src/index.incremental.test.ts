...
describe('@exactjs/expressions: incremental', () => {
	it('keeps earlier module versions and analyses immutable', () => {
		...
		const second = project.updateModule(
			filename,
			'// unrelated insertion\nlet value = 1; value += 3;'
		);
		...
		const firstVariable = first
			.walk()
			.references()
			.first((ref) => ref.name === 'value')!.variable!;
	...
	it('reuses analyses for structurally shared unchanged subtrees', () => {
		...
		const module = project.updateModule(
			filename,
			'const outer = 1; function first() { return () => outer; } function second() { return 2; }'
		);
		const first = module
			.walk()
			.functions()
			.where((reference) => reference.node.name === 'first')
			.single();
		...
		const rewritten = rewriteModule(module, (rewriter) =>
			rewriter.replaceTextWhere(
				(reference) => reference.node.text === '2',
				() => '3'
			)
		);
	...
	it('uses host filesystem casing rules for incremental overlays', () => {
		...
		if (process.platform === 'win32')
			expect(project.getModule(lower).emit().code).toContain('upper = 2');
	});
	...
	it('assigns collision-free identities to same-named sibling block bindings', () => {
		...
		const module = project.updateModule(
			filename,
			`function view(flag: boolean) { if (flag) { const value = 1; void value; } else { const value = 2; void value; } }`
		);
		const values = module
			.walk()
			.references()
		...
		const updated = project.updateModule(
			filename,
			`function view(flag: boolean) { if (flag) { const value = 100; void value; } else { const value = 2; void value; } }`
		);
		expect(
			updated
				.walk()
	...
	it('does not transfer identities between ambiguous repeated control scopes', () => {
		...
		const first = project.updateModule(
			filename,
			`function view(a: boolean, b: boolean) {
      ...
      if (b) { const value = 2; void value; }
		...
		const oldIds = new Set(
			first
				...
				.where((reference) => reference.name === 'value')
				.toArray()
				.map((reference) => reference.variable!.id)
		);
