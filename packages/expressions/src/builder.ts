...
export class TypeBuilder {
	...
	array(element: ExpressionType): ExpressionType {
		return type('object', `readonly ${element.display}[]`);
	}
	mutableArray(element: ExpressionType): ExpressionType {
		return type('object', `${element.display}[]`);
	}
	...
	nullable(value: ExpressionType): ExpressionType {
		return this.union(value, type('null'));
	}
	...
	literal(value: string | number | bigint | boolean | null | undefined): ExpressionNode {
		const valueType =
			value === null
				...
				: value === undefined
					? type('undefined')
					: type(typeof value as ExpressionTypeKind);
		const text =
			typeof value === 'string'
				? JSON.stringify(value)
				: typeof value === 'bigint'
