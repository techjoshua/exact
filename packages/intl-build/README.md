...
source of truth. `synchronizeXliff21Catalog()` refreshes source units while preserving target inline
codes, notes, review state, and obsolete history.

The interchange is schema-valid XLIFF 2.1. eXact's bounded runtime metadata is stored in core
`originalData` entries and referenced by `dataRef`/`dataRefStart`; required `<ph>` and `<pc>` codes
cannot be copied or deleted. Custom eXact identifiers use only XLIFF's user-defined `subType` and
marker `type` slots, so the format does not rely on foreign XML attributes that translation tools
may strip.
