 ...
 */
export function sameJsonData(
	left: unknown,
	right: unknown,
	options: JsonComparisonOptions = {}
): boolean {
	type Comparison = {
		readonly kind: 'compare' | 'complete';
	...
	while (pending.length) {
		...
		if (!isJsonObject(currentLeft) || !isJsonObject(currentRight)) {
			if (
				!isJsonScalar(currentLeft) ||
				!isJsonScalar(currentRight) ||
				!Object.is(currentLeft, currentRight)
			)
				return false;
			continue;
		}
		...
		if (Array.isArray(currentLeft)) {
			...
			if (
				Object.getOwnPropertySymbols(currentLeft).some((symbol) =>
					Object.prototype.propertyIsEnumerable.call(currentLeft, symbol)
				) ||
				Object.getOwnPropertySymbols(rightArray).some((symbol) =>
					Object.prototype.propertyIsEnumerable.call(rightArray, symbol)
				)
			)
				return false;
			if (
				Object.keys(currentLeft).some((key) => !isArrayIndexKey(key, currentLeft.length)) ||
				Object.keys(rightArray).some((key) => !isArrayIndexKey(key, rightArray.length))
			)
				return false;
		...
		if (
			...
			Object.getOwnPropertySymbols(currentRight).some((symbol) =>
				Object.prototype.propertyIsEnumerable.call(currentRight, symbol)
			)
		)
			return false;
		const leftKeys = Object.keys(leftDescriptors)
			.filter((key) => leftDescriptors[key]!.enumerable)
			.sort();
		const rightKeys = Object.keys(rightDescriptors)
			.filter((key) => rightDescriptors[key]!.enumerable)
			.sort();
		if (
