...
describe('bounded JSON equality', () => {
	it('compares property-order-independent JSON without requiring shared identity', () => {
		...
		expect(
			sameJsonData({ right: shared, left: shared }, { left: { value: 1 }, right: { value: 1 } })
		).toBe(true);
	});
	...
	it('fails closed for cycles, getters, sparse arrays, and non-JSON scalars', () => {
		...
		expect(
			sameJsonData(Object.defineProperty({}, 'value', { enumerable: true, get: () => 1 }), {
				value: 1
			})
		).toBe(false);
		expect(
			sameJsonData(Object.defineProperty([], '0', { enumerable: true, get: () => 1 }), [1])
		).toBe(false);
