...
function observeLifecyclePromise(
	instance: ComponentInstance<any>,
...
function observeTaskPromise(promise: Promise<unknown>, instance: ComponentInstance<any>): void {
	(
		taskObserverStack[taskObserverStack.length - 1] ?? retainedTaskObservers.get(instance)
	)?.register(promise, instance);
}
...
function createComponentReactiveValue<T>(
	instance: ComponentInstance<any>,
