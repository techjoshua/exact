...
function inspectMounted(mounted: Mounted, parent: DomInspectionNode | undefined): DomInspectionNode {
  let children: readonly DomInspectionNode[] = [];
  const elements = Object.freeze(rootElements(mounted));
  const owned = Object.freeze(mounted.instance ? allElements(mounted).filter(element => elementOwners.get(element) === mounted.instance) : []);
...
function isElementNode(node: Node): node is Element { return node.nodeType === 1; }
