...
export function namespaceForTag(tag: string, parent?: Element): string | undefined {
  ...
  if (inherited === SVG_NAMESPACE) return parent.localName === "foreignObject" ? undefined : SVG_NAMESPACE;
  if (inherited !== MATHML_NAMESPACE) return undefined;
  if (["mi", "mo", "mn", "ms", "mtext"].includes(parent.localName)
    && tag !== "mglyph" && tag !== "malignmark") return undefined;
