...
export function preserveFocus<T>(root: Root, work: () => T): T {
  ...
  if (active?.isConnected && document.activeElement === active) {
    if (inputSelection && (active instanceof HTMLInputElement || active instanceof HTMLTextAreaElement)
...
function cloneSelection(root: HTMLElement): SavedSelection | undefined {
  ...
  return {
    start: pathFrom(root, range.startContainer), startOffset: range.startOffset,
    end: pathFrom(root, range.endContainer), endOffset: range.endOffset
  };
}
...
function restoreSelection(root: HTMLElement, saved: SavedSelection): void {
  ...
  } catch { /* The edited content no longer contains a compatible range. */ }
}
