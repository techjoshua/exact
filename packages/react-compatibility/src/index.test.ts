...
import { capabilityFor, compareConformanceTraces, reactCapabilities, rendererCompatibilityCapabilities, type ConformanceTrace } from "./index.js";
...
describe("React compatibility contract", () => {
  ...
  it("publishes renderer capabilities and 100 machine-readable package/version results", () => {
    expect(rendererCompatibilityCapabilities.find(item => item.name === "generic-react-reconciler")?.status).toBe("supported");
    const corpus = JSON.parse(readFileSync(new URL("../ordinary-package-corpus.json", import.meta.url), "utf8")) as {
    ...
    expect(new Set(corpus.packages.map(item => `${item.package}@${item.version}`)).size).toBe(corpus.packages.length);
  ...
  it("reports structural trace differences and ignores baseline metadata", () => {
    const trace = (baseline: "18.3" | "19.2", updatedHtml = "<button>1</button>"): ConformanceTrace => ({
      baseline,
      version: baseline,
      exports: {},
      element: { type: "div", key: "item", children: ["A", "B"] },
      serverHtml: "<p>server</p>",
      client: { initialHtml: "<button>0</button>", updatedHtml, renders: 2, events: ["layout:0", "effect:0"] }
    });
