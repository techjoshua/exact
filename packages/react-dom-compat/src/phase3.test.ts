...
import { Activity, act, cache, createElement, useActionState, useOptimistic } from "@exact/react-compat";
...
describe("React compatibility Phase 3", () => {
  ...
  it("runs action and optimistic hooks and resets optimistic state from passthrough props", async () => {
    function Actions(props: { base: number }) {
      const [state, dispatch, pending] = useActionState(async (previous: number, amount: number) => previous + amount, 0);
      const [optimistic, updateOptimistic] = useOptimistic(props.base, (previous, amount: number) => previous + amount);
      const status = useFormStatus();
      return createElement("div", null,
        createElement("button", { id: "action", onClick: () => dispatch(2) }, `${state}/${pending}`),
        createElement("button", { id: "optimistic", onClick: () => updateOptimistic(3) }, String(optimistic)),
  ...
  it("supports Activity visibility, resource hints, and form reset", async () => {
    ...
    expect(document.head.querySelector("script[type=module]")?.getAttribute("src")).toContain("/app.js");
    ...
    const container = document.createElement("div");
    const root = createRoot(container);
    await act(() => root.render(createElement(Activity, { mode: "hidden" }, createElement("span", null, "hidden"))));
    expect(container.textContent).toBe("");
    await act(() => root.render(createElement(Activity, { mode: "visible" }, createElement("span", null, "visible"))));
