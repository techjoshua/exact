...
import { ReactCacheContext, ReactRootContext, reactCompatibilityTarget, toExactNode, type ReactCacheScope, type ReactRootRuntime } from "@exact/react-compat/exact";
import type { ReactNode } from "@exact/react-compat";
import { renderToString as renderExactToString, renderToStringAsync as renderExactToStringAsync } from "@exact/ssr";
...
type ServerRootProps = { node: ReactNode; options: ServerRenderOptions; resources: Map<string, { priority: number; html: string }> };
const ServerRoot = function ExactReactServerRoot(this: import("@exact/core").Component<Record<string, never>>, props: ServerRootProps) {
  const scope: ReactCacheScope = { roots: new Map(), controller: new AbortController() };
  const runtime: ReactRootRuntime = { identifierPrefix: props.options.identifierPrefix ?? "", nextComponentId: 0, resources: props.resources };
  this.setContext(ReactCacheContext, scope);
  this.setContext(ReactRootContext, runtime);
  this.onUnmount(() => scope.controller.abort(new DOMException("React server cache request completed", "AbortError")));
  return () => toExactNode(props.node);
} as ComponentFunction<Record<string, never>, ServerRootProps>;

function serverVNode(node: ReactNode, options: ServerRenderOptions, resources: Map<string, { priority: number; html: string }>) {
  return createVNode(ServerRoot, { node, options, resources });
}

export function renderReactToString(node: ReactNode, options: ServerRenderOptions = {}, textSeparators = true): string {
  try {
...
export async function renderReactToStringAsync(node: ReactNode, options: ServerRenderOptions = {}, textSeparators = true): Promise<string> {
  try {
    const resources = new Map<string, { priority: number; html: string }>();
    const html = (await renderExactToStringAsync(serverVNode(node, options, resources), {
      markers: false,
      reactMarkup: reactCompatibilityTarget(),
      textSeparators,
      signal: options.signal
...
export async function renderToReadableStream(node: ReactNode, options: ServerRenderOptions = {}): Promise<ReadableStream<Uint8Array> & { allReady: Promise<void> }> {
  const html = withBootstrapScripts(await renderReactToStringAsync(node, options), options);
  const stream = readableStreamFromString(html) as ReadableStream<Uint8Array> & { allReady: Promise<void> };
...
export function withBootstrapScripts(html: string, options: ServerRenderOptions): string {
  ...
  const preloads = target === 19
    ? [
        ...scripts.map(src => `<link rel="preload" as="script" fetchPriority="low"${nonce} href="${escapeHtmlAttribute(src)}"/>`),
        ...modules.map(src => `<link rel="modulepreload" fetchPriority="low"${nonce} href="${escapeHtmlAttribute(src)}"/>`)
      ].join("")
    : "";
  const inline = options.bootstrapScriptContent === undefined
    ? ""
  ...
  const suffix = [
    inline,
    ...scripts.map(src => `<script src="${escapeHtmlAttribute(src)}"${externalNonce} async=""></script>`),
    ...modules.map(src => `<script type="module" src="${escapeHtmlAttribute(src)}"${externalNonce} async=""></script>`)
  ].join("");
