...
import { Component, Suspense, createElement, lazy, useId, type ReactNode } from "@exact/react-compat";
...
import { preconnect, prefetchDNS, preinit, preinitModule, preload, preloadModule } from "./index.js";
...
describe("React compatibility Phase 6 root hardening", () => {
  ...
  it("routes caught and uncaught failures through React root callbacks", () => {
    ...
    class Boundary extends Component<{ children?: ReactNode }, { error?: Error }> {
      ...
      render() { return this.state.error ? createElement("strong", null, "caught") : this.props.children; }
    }
    function Broken(): never { throw new Error("boom"); }

    const caughtContainer = document.createElement("div");
    createRoot(caughtContainer, { onCaughtError: caught }).render(createElement(Boundary, null, createElement(Broken)));
  ...
  it("normalizes specialized form, SVG, custom-element, style, and image server markup", () => {
    const tree = createElement("section", null,
      ...
      createElement("select", { value: "b", readOnly: true },
        createElement("option", { value: "a" }, "A"),
        createElement("option", { value: "b" }, "B")
      ),
      createElement("svg", { viewBox: "0 0 1 1", strokeWidth: 2 }, createElement("use", { xlinkHref: "#x" })),
  ...
  it("emits React 19 bootstrap preloads and scripts on pipeable streams", async () => {
    ...
    const ended = new Promise<void>((resolve, reject) => destination.on("end", resolve).on("error", reject));
  ...
  it("waits for Suspense resources in all-ready streams and supports abort", async () => {
    const LazyValue = lazy(() => Promise.resolve({ default: () => createElement("b", null, "ready") }));
    const stream = await renderToReadableStream(
      createElement(Suspense, { fallback: createElement("i", null, "loading") }, createElement(LazyValue))
    );
  ...
  it("preserves user-edited form state when a hydration mismatch is replaced", () => {
    ...
    hydrateRoot(container, createElement("section", null,
      createElement("input", { name: "email", defaultValue: "client" })
    ));
    expect(container.querySelector<HTMLInputElement>('input[name="email"]')?.value).toBe("user@example.test");
  });
});
