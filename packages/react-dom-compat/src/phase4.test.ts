...
import { Component, Fragment, Profiler, PureComponent, act, createContext, createElement, createRef } from "@exact/react-compat";
...
describe("React compatibility Phase 4", () => {
  it("preserves class identity, state, context, refs, and update lifecycles", async () => {
    ...
    class Counter extends Component<{ label: string }, { count: number }> {
      ...
      componentDidMount() { events.push(`mount:${this.context}`); }
      getSnapshotBeforeUpdate(_props: { label: string }, state: { count: number }) { return `snapshot:${state.count}`; }
      ...
      render() {
        return createElement("button", { onClick: () => this.setState(value => ({ count: value.count + 1 }), () => events.push("callback")) },
    ...
    await act(() => root.render(createElement(Tone.Provider, { value: "dark" }, createElement(Counter, { ref, label: "first" }))));
    ...
    await act(() => root.render(createElement(Tone.Provider, { value: "light" }, createElement(Counter, { ref, label: "second" }))));
  ...
  it("honors shouldComponentUpdate, forceUpdate, and PureComponent shallow comparison", async () => {
    ...
    class Controlled extends Component<{ value: number }> {
      shouldComponentUpdate(next: { value: number }) { return next.value % 2 === 0; }
      render() { renders(this.props.value); return createElement("span", null, String(this.props.value)); }
    }
    ...
    class PureLabel extends PureComponent<{ value: number }> {
      render() { pureRenders(); return createElement("b", null, String(this.props.value)); }
    }
  ...
  it("routes descendant errors to class boundaries without letting a boundary catch itself", async () => {
    ...
    class Boundary extends Component<{ children?: unknown }, { error: Error | null }> {
      ...
      componentDidCatch(error: Error, info: { componentStack: string }) { caught(error.message, info.componentStack); }
      render() { return this.state.error ? createElement("strong", null, this.state.error.message) : this.props.children as never; }
    }
    function Broken(): never { throw new Error("child failed"); }
    const container = document.createElement("div");
    await act(() => createRoot(container).render(createElement(Boundary, null, createElement(Broken, null))));
    expect(container.textContent).toBe("child failed");
    expect(caught).toHaveBeenCalledTimes(1);

    class SelfBrokenBoundary extends Boundary { render(): never { throw new Error("boundary failed"); } }
    const nested = document.createElement("div");
    await act(() => createRoot(nested).render(createElement(Boundary, null, createElement(SelfBrokenBoundary, null))));
  ...
  it("reports Profiler mount and update commits", async () => {
    ...
    const container = document.createElement("div");
    const root = createRoot(container);
    await act(() => root.render(createElement(Profiler, { id: "profile", onRender }, createElement("span", null, "first"))));
    await act(() => root.render(createElement(Profiler, { id: "profile", onRender }, createElement("span", null, "second"))));
    expect(onRender.mock.calls.map(call => call.slice(0, 2))).toEqual([["profile", "mount"], ["profile", "update"]]);
  });
  ...
  it("propagates legacy child context through class components", async () => {
    class Provider extends Component<{ children?: unknown }> {
      ...
      getChildContext() { return { tone: "legacy-dark" }; }
      render() { return this.props.children as never; }
    }
    class Reader extends Component {
      static contextTypes = { tone: () => null };
      declare context: { tone?: string };
      render() { return createElement("span", null, this.context.tone); }
    }
    const container = document.createElement("div");
    await act(() => createRoot(container).render(createElement(Provider, null, createElement(Reader, null))));
    expect(container.textContent).toBe("legacy-dark");
  });

  it("findDOMNode resolves host, fragment, and unmounted class ownership", async () => {
    class Host extends Component { render() { return createElement("button", null, "host"); } }
    class FragmentHost extends Component { render() { return createElement(Fragment, null, "text", createElement("i", null, "tail")); } }
  ...
  it("releases class ownership and refs even when componentWillUnmount throws", async () => {
    class BrokenUnmount extends Component {
      componentWillUnmount() { throw new Error("unmount failure"); }
      render() { return createElement("button", null, "owned"); }
    }
