...
describe("React compatibility Phase 2", () => {
  ...
  it("propagates reactive context through Provider and Consumer forms", async () => {
    ...
    function HookReader() { return createElement("span", { id: "hook" }, useContext(Theme)); }
    function ConsumerReader() {
      return createElement(Theme.Consumer, null, (value: string) => createElement("span", { id: "consumer" }, value));
    }
    const view = (value: string) => createElement(Theme.Provider, { value }, createElement(HookReader, null), createElement(ConsumerReader, null));
    const container = document.createElement("div");
  ...
  it("orders insertion, layout, and passive setup/cleanup deterministically", async () => {
    ...
    function Effects() {
      ...
      useInsertionEffect(() => { events.push(`insertion:${value}`); return () => events.push(`insertion-cleanup:${value}`); }, [value]);
      useLayoutEffect(() => { events.push(`layout:${value}`); return () => events.push(`layout-cleanup:${value}`); }, [value]);
      useEffect(() => { events.push(`passive:${value}`); return () => events.push(`passive-cleanup:${value}`); }, [value]);
      return createElement("button", { onClick: () => setValue(next => next + 1) }, String(value));
    }
    const container = document.createElement("div");
    const root = createRoot(container);
    ...
    expect(events).toEqual([
      "insertion:0", "layout:0", "passive:0",
      "insertion-cleanup:0", "insertion:1", "layout-cleanup:0", "layout:1", "passive-cleanup:0", "passive:1"
    ]);
    await act(() => root.unmount());
    expect(events.slice(-3)).toEqual(["insertion-cleanup:1", "layout-cleanup:1", "passive-cleanup:1"]);
  });

  it("runs every hook cleanup even when an earlier cleanup throws", async () => {
    const cleanups: string[] = [];
    function BrokenCleanup() {
      useEffect(() => () => { cleanups.push("first"); throw new Error("cleanup failure"); }, []);
      useEffect(() => () => { cleanups.push("second"); }, []);
  ...
  it("fulfills object, callback, forwarded, and imperative refs", async () => {
    ...
    await act(() => root.render(createElement("div", null,
      createElement("span", { ref: (value: Element | null) => callbackValues.push(value) }, "callback"),
  ...
  it("subscribes to external stores and unsubscribes on unmount", async () => {
    ...
    const subscribe = vi.fn((listener: () => void) => { listeners.add(listener); return () => listeners.delete(listener); });
    const getSnapshot = () => value;
    function StoreView() { return createElement("span", null, String(useSyncExternalStore(subscribe, getSnapshot))); }
    const container = document.createElement("div");
    const root = createRoot(container);
    await act(() => root.render(createElement(StoreView, null)));
    expect(container.textContent).toBe("0");
    value = 1;
    await act(() => { for (const listener of listeners) listener(); });
