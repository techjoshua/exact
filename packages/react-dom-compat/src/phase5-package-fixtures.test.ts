...
describe("React compatibility Phase 5 package fixtures", () => {
  it("server-renders Emotion context without running client effects", () => {
    ...
    expect(renderToString(createElement(ThemeProvider, { theme: { brand: "server-blue" } }, createElement(ThemeValue))))
  ...
  it("server-renders a TanStack Query with seeded request data", () => {
    ...
    expect(renderToString(createElement(QueryClientProvider, { client }, createElement(QueryValue))))
