...
const RootHost = function ExactReactRoot(this: Component<Record<string, never>>, props: RootHostProps) {
...
class CompatibilityRoot implements Root {
  ...
  constructor(private readonly container: Element, private readonly options?: RootOptions, private hydration?: HydrationRoot) {}
...
export function createRoot(container: Element | DocumentFragment, _options?: RootOptions): Root {
  if (!(container instanceof Element)) {
    throw new TypeError("eXact React compatibility Phase 1 createRoot requires an Element container");
  }
  if (roots.has(container)) throw new Error("createRoot has already been called for this container");
...
export function hydrateRoot(container: Element | DocumentFragment, initialChildren: ReactNode, options?: RootOptions): Root {
  ...
  if (roots.has(container)) throw new Error("A React compatibility root already owns this container");
...
export function legacyHydrate(children: ReactNode, container: Element, callback?: () => void): null {
  let root = roots.get(container);
