...
describe('React compatibility server rendering', () => {
	it('matches common React string markup conventions', () => {
		...
		const html =
			'<label for="field" tabindex="0" disabled="" spellCheck="false" style="background-color:red;line-height:2;margin-top:4px">first<!-- -->second</label>';
	...
	it('scopes React cache entries and cache signals to one server request', () => {
		...
		expect(
			renderToString(createElement('div', null, createElement(Value), createElement(Value)))
