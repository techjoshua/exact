...
beforeAll(async () => {
  ({ Dialog, ThemeProvider, useTheme, QueryClient, QueryClientProvider, useQuery } = await import("../fixtures/phase3.mjs"));
});
...
describe("React compatibility Phase 3 package fixtures", () => {
  it("opens and cleans up a Radix Dialog portal", async () => {
    ...
    await act(() => root.render(createElement(Dialog.Root, null,
      createElement(Dialog.Trigger, null, "open"),
      createElement(Dialog.Portal, null,
        createElement(Dialog.Overlay, { "data-testid": "overlay" }),
        createElement(Dialog.Content, null,
    ...
    await act(() => Array.from(document.body.querySelectorAll("button")).find(button => button.textContent === "close")!.click());
  ...
  it("runs Emotion context and insertion-effect setup", async () => {
    ...
    const container = document.createElement("div");
    await act(() => createRoot(container).render(
      createElement(ThemeProvider, { theme: { brand: "exact-blue" } }, createElement(ThemeValue, null))
    ));
  ...
  it("resolves a suspense-enabled TanStack Query", async () => {
    ...
    const result = new Promise<string>(settle => { resolve = settle; });
