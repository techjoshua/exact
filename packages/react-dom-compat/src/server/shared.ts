...
import { reactCompatibilityTarget } from '@exactjs/react-compat/exact';
import { renderReactServerTree, renderReactServerTreeAsync } from '../renderer/server.js';
...
export function renderReactToString(
	...
	try {
		const { html, resources } = renderReactServerTree(node, options, textSeparators);
...
export async function renderReactToStringAsync(
	...
	try {
		const { html, resources } = await renderReactServerTreeAsync(node, options, textSeparators);
		return `${renderResourceHints(resources)}${html}`;
	} catch (error) {
		options.onError?.(error, { componentStack: '' });
		throw error;
