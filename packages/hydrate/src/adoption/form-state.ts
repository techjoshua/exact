import { consumeDomWork, walkDomSubtree, type DomWorkBudget } from "@exact/dom";
...
export function restoreFormState(container: Element, states: readonly FormState[], work: DomWorkBudget): void {
  if (!states.length) return;
  const identities = indexFormControlIdentities(container, work);
  for (const state of states) {
    const identityMatch = state.identity ? identities.get(`${state.identity.attribute}\0${state.identity.value}`) : undefined;
    ...
    const control = candidate instanceof Element && formControlSignature(candidate) === state.signature ? candidate : undefined;
