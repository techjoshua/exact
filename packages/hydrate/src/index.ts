import { adoptComponentRoot, adoptDocumentRoot, adoptMarkerlessComponentRoot, adoptStatic, consumeDomWork, createDomWorkBudget, render, unmount, type DomWorkBudget } from "@exact/dom";
...
function hydrateRoot(vnode: VNode, container: Element | Document, options: HydrateOptions): HydrationRoot {
  ...
  if (existing) {
    render(vnode, rootContainer, { logger: options.logger, onErrorReport: options.onErrorReport, maxTreeDepth: options.maxTreeDepth, maxTreeNodes: options.maxTreeNodes, allowUnsafeHtml: options.allowUnsafeHtml, onUnsafeHtml: options.onUnsafeHtml, onProfile: options.onProfile });
    return existing;
  }
  ...
  if (!captured.hasMarkers) {
    ...
    render(vnode, rootContainer, { logger: resolvedOptions.logger, onErrorReport: resolvedOptions.onErrorReport, maxTreeDepth: resolvedOptions.maxTreeDepth, maxTreeNodes: remainingDomWork(work), workBudget: work, allowUnsafeHtml: resolvedOptions.allowUnsafeHtml, onUnsafeHtml: resolvedOptions.onUnsafeHtml, onProfile: options.onProfile });
  } else {
    if ((typeof vnode.type === "function"
      ...
      : adoptStaticTree(vnode, rootContainer, createStaticAdoptionBudget(resolvedOptions, work)) && adoptStatic(vnode, rootContainer, {
        logger: resolvedOptions.logger,
        onErrorReport: resolvedOptions.onErrorReport,
        maxTreeDepth: resolvedOptions.maxTreeDepth,
        maxTreeNodes: remainingDomWork(work),
