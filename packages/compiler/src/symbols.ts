...
export function createValueRootSymbols(
	source: SourceIdentity,
	callables: readonly ExactCallableSummaryIR[]
): ExactSymbolIR[] {
		...
		for (const exportName of callable.exportNames) {
			...
			const placement =
				...
				(callable.artifactTargets.length === 1 && callable.artifactTargets[0] === 'client')
					...
					: callable.effect === 'server' ||
						  (callable.artifactTargets.length === 1 && callable.artifactTargets[0] === 'server')
						? 'server'
			...
			const localName =
				callable.kind === 'initializer'
					? callable.name.replace(/\.initializer$/, '')
					: callable.name;
			symbols.push({
...
export function createRootSymbols(
	source: SourceIdentity,
...
export function createServerPartSymbols(
	source: SourceIdentity,
	components: ExactComponentIR[]
): ExactSymbolIR[] {
	const filename = sourceFilename(source);
...
export function createClientIslandSymbols(
	source: SourceIdentity,
	components: ExactComponentIR[]
): ExactSymbolIR[] {
	const filename = sourceFilename(source);
