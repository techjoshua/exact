...
describe('expression-backed derived substitutions', () => {
	it('plans safe immutable reads by canonical variable identity', () => {
		...
		const module = expressionModuleFor(
			'DerivedPlan.tsx',
			`function View(this: Component<{ value: number }>) {
      ...
      return () => <p>{doubled + mutable}</p>;
		...
		const plannedText = [...plan.sites.values()].map((site) =>
			module.source.slice(site.start, site.end)
		);
		expect(plannedText).toEqual(['doubled']);
		expect(
			module.source.slice(
				[...plan.sites.values()][0]!.initializerStart,
				[...plan.sites.values()][0]!.initializerEnd
			)
		).toBe('this.state.value * 2');
		expect([...plan.declarations.values()]).toContainEqual(
