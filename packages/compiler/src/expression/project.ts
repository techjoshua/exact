...
export class ExactCompilerSession {
	...
	expressionModuleFor(
		...
		const config =
			findExpressionConfig(path.dirname(absolute)) ?? findExpressionConfig(root ?? process.cwd());
		if (!config) {
		...
		const projectKey = virtual
			? `${configuredKey}::virtual-root:${canonical(root!)}`
			: configuredKey;
	...
	expressionDependencyFiles(
		...
		const target = canonical(
			(options.virtual ?? !path.isAbsolute(filename))
				? this.relativeLocation(filename, options.root).absolute
				: filename
		);
		const entry = [...this.modules.values()].find(
			(candidate) => canonical(candidate.filename) === target && candidate.source === source
		);
	...
	private synchronizeLanguageServiceFile(
		filename: string,
		removed: boolean
	...
	private invalidateLanguageServiceAffected(
		update: ExpressionLanguageServiceUpdate,
		changed: string
	): void {
		for (const affected of update.affectedFiles) {
	...
	private relativeLocation(
		filename: string,
		explicitRoot?: string
	): Readonly<{ root: string; absolute: string }> {
		if (explicitRoot) {
	...
	private collectDependentModuleKeys(filename: string, seen = new Set<string>()): Set<string> {
		...
		for (const moduleKey of this.dependents.get(target) ?? []) {
			...
			const entry = this.modules.get(moduleKey);
			if (entry)
				for (const nested of this.collectDependentModuleKeys(entry.filename, seen))
