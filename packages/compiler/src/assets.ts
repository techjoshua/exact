...
export function analyzeModuleImports(
  ...
  const sourceFile = ts.createSourceFile(filename, source, ts.ScriptTarget.ES2022, true, scriptKind(filename));
  ...
  for (const statement of sourceFile.statements) {
    ...
    if (placement) {
      ...
      if (prior && prior !== placement) diagnostics.push(`error: import '${specifier}' has conflicting exact placement attributes`);
    ...
    const evaluationTarget = placement ?? rule.evaluationTarget ?? defaultEvaluationTarget(importMode, sideEffect);
    ...
    if (sideEffect && evaluationTarget === "client") clientAssetSideEffectStarts.add(statement.getStart(sourceFile));
    if (!placement && evaluationTarget !== "both") placementBySpecifier.set(specifier, evaluationTarget);
  }
...
export function stripExactImportAttribute(node: ts.ImportDeclaration, factory: ts.NodeFactory): ts.ImportDeclaration {
  const attributes = node.attributes;
  if (!attributes) return node;
  const retained = attributes.elements.filter(element => importAttributeName(element.name) !== "exact");
  if (retained.length === attributes.elements.length) return node;
  const next = retained.length
    ? factory.updateImportAttributes(attributes, factory.createNodeArray(retained), attributes.multiLine)
    : undefined;
  return factory.updateImportDeclaration(node, node.modifiers, node.importClause, node.moduleSpecifier, next);
}
...
function exactImportPlacement(
  ...
  const matches = node.attributes?.elements.filter(element => importAttributeName(element.name) === "exact") ?? [];
  if (!matches.length) return undefined;
  const location = sourceFile.getLineAndCharacterOfPosition(node.getStart(sourceFile));
  const prefix = `${sourceFile.fileName}:${location.line + 1}:${location.character + 1}`;
  if (matches.length > 1) diagnostics.push(`error: ${prefix} import has duplicate exact attributes`);
  if (node.importClause?.isTypeOnly) diagnostics.push(`error: ${prefix} type-only import cannot declare exact placement`);
  const value = matches[0]?.value;
  if (!value || !ts.isStringLiteral(value) || (value.text !== "client" && value.text !== "server")) {
...
function matchingAssetRule(specifier: string, rules: readonly ExactAssetRule[]): ExactAssetRule | undefined {
  const [pathname, query = ""] = specifier.split("?", 2);
  const clean = pathname!.split("#", 1)[0]!.toLowerCase();
  const queryTokens = new Set(query.split("&").map(token => token.split("=", 1)[0]!.toLowerCase()).filter(Boolean));
  return rules.find(rule => {
    const extensionMatch = !rule.extensions?.length || rule.extensions.some(extension => clean.endsWith(normalizeExtension(extension)));
    const queryMatch = !rule.queries?.length || rule.queries.some(token => queryTokens.has(token.replace(/^\?/, "").toLowerCase()));
...
function defaultEvaluationTarget(mode: ExactAssetImportMode, sideEffect: boolean): Exclude<ExactAssetTarget, "embedded"> {
