...
export function exactComponentDescriptorTransformer(
  ...
  for (const component of manifest.components) {
    const generatedEntries = manifest.symbols.filter(symbol =>
      symbol.componentId === component.id
      && symbol.target === target
      && !!symbol.exportName
      && symbol.role === (target === "client" ? "client-island" : "server-part")
    );
    const rootEntry = target === "client" && component.placement === "client"
      ? manifest.symbols.find(symbol =>
        symbol.componentId === component.id
    ...
    if (entries.length) groups.set(component.name, {
  ...
  return context => sourceFile => {
    ...
    const descriptorSymbol = context.factory.createUniqueName(
      target === "client"
        ? "__exactClientComponentDescriptor"
        : "__exactServerComponentDescriptor"
    );
    for (const statement of sourceFile.statements) {
      ...
      statements.push(...(preserveHoisting
        ? attachToHoistedComponentFunction(statement, groups.get(statement.name.text)!, descriptorSymbol, context.factory)
        : wrapComponentFunction(statement, groups.get(statement.name.text)!, descriptorSymbol, context.factory)));
    }
    ...
    while (insertionIndex < statements.length) {
      ...
      if (!ts.isImportDeclaration(statement)
        && !(ts.isExpressionStatement(statement) && ts.isStringLiteral(statement.expression))) break;
      insertionIndex++;
    }
...
function wrapComponentFunction(
  ...
  const defaultExport = declaration.modifiers?.some(modifier => modifier.kind === ts.SyntaxKind.DefaultKeyword) ?? false;
...
function descriptorAttachment(
  ...
  return pure(factory.createCallExpression(
  ...
  return factory.createVariableStatement(
    ...
    factory.createVariableDeclarationList([
      factory.createVariableDeclaration(
        ...
        undefined,
        undefined,
        pure(factory.createCallExpression(
