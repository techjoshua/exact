...
it.each([false, true])(
	'issues nested server tasks before serialization (keyed list: %s)',
	async (keyed) => {
		const compiled = transform(
			...
			function Leaf(this: Component<State>, props: { value: number }) {
				...
				return () => ${keyed ? '<section>{[1].map(item => <span key={item}>{this.state.value}</span>)}</section>' : '<span>{this.state.value}</span>'};
			}
