...
describe("compiler HMR session retention", () => {
  it("plateaus retained module state across repeated edits and releases removed overlays", () => {
    ...
    const compile = (revision: number) => transformSource(`
  ...
  it("isolates owned sessions and rejects work after disposal", () => {
    ...
    const options = { filename: "src/session.ts", root: path.resolve(import.meta.dirname, "../../..") };
    first.expressionModuleFor(options.filename, "export const value = 1;", options);
    const retained = second.expressionModuleFor(options.filename, "export const value = 2;", options);

    first.dispose();
    expect(() => first.expressionModuleFor(options.filename, "export const value = 3;", options)).toThrow("disposed");
    expect(second.expressionModuleFor(options.filename, "export const value = 2;", options)).toBe(retained);
    second.dispose();
  });
  ...
  it("removes a deleted file's stable identities as well as its overlay", () => {
    ...
    session.expressionModuleFor(filename, "export const removedIdentity = 1; void removedIdentity;");
