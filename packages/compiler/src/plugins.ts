...
export function applyCompilerPlugins(
  ...
  const byNamespace = new Map(Object.values(registry?.plugins ?? {}).flatMap(plugin =>
  ...
  for (const directive of directives) {
    ...
    if (!supported.includes(directive.name)) {
      throw new Error(`${filename}: unknown @exact ${directive.namespace}.${directive.name} directive`);
    }
  }
  const diagnostics: string[] = [];
  const pluginData: Record<string, ExactJsonValue> = {};
  for (const plugin of Object.values(registry?.plugins ?? {}).sort((left, right) => left.packageName.localeCompare(right.packageName))) {
    ...
    const ownedDirectives = directives.filter(directive => directive.namespace === extension.namespace);
    const contribution = extension.analyzeModule(Object.freeze({
      id: filename,
      source,
      target,
    ...
    for (const diagnostic of contribution?.diagnostics ?? []) {
      diagnostics.push(`${diagnostic.severity}: [${plugin.packageName}/${diagnostic.code}] ${diagnostic.message}`);
    }
  ...
  return {
    pluginRegistry: {
      ...
      plugins: Object.fromEntries(Object.values(registry.plugins)
  ...
  for (const manifest of manifests) {
    ...
    for (const [name, metadata] of Object.entries(imported.plugins)) {
      ...
      if (metadata.required && (!plugin || plugin.protocolVersion !== metadata.protocolVersion)) {
        throw new Error(`Imported compiler manifest ${manifest.filename} requires incompatible plugin ${name}`);
      }
    }
  }
}
...
export function collectPluginDirectives(source: string): ExactCompilerDirective[] {
  ...
  const marker = /@exact\s+([A-Za-z_$][\w$-]*)\.([A-Za-z_$][\w$-]*)(?:\s*=\s*(?:"([^"]*)"|'([^']*)'|([A-Za-z_$][\w$]*)))?/g;
  for (let match = marker.exec(source); match; match = marker.exec(source)) {
    result.push(Object.freeze({
      namespace: match[1]!,
      name: match[2]!,
      ...(match[3] ?? match[4] ?? match[5] ? { argument: match[3] ?? match[4] ?? match[5] } : {}),
      start: match.index,
