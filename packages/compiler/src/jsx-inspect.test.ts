...
function printChildrenProp(text: string): string | undefined {
  ...
  ts.transform(file, [context => root => {
...
describe("jsx inspection helpers", () => {
  it("detects client islands from event handlers and refs", () => {
    expect(jsxElementIsClientIsland(firstSelfClosingElement("const x = <Panel onClick={save} />;").attributes)).toBe(true);
    expect(jsxElementIsClientIsland(firstSelfClosingElement("const x = <Panel ref={node} />;").attributes)).toBe(true);
    expect(jsxElementIsClientIsland(firstSelfClosingElement("const x = <Panel title=\"Work\" />;").attributes)).toBe(false);
  });

  it("classifies intrinsic tags without treating components as intrinsic", () => {
    expect(jsxTagIsIntrinsicElement(firstSelfClosingElement("const x = <div />;").tagName)).toBe(true);
    expect(jsxTagIsIntrinsicElement(firstSelfClosingElement("const x = <Panel />;").tagName)).toBe(false);
  });
