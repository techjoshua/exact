...
describe("symbol-level placement inference", () => {
  it("propagates server effects through local helper chains", () => {
    ...
    expect(task.effectSources).toContainEqual(expect.objectContaining({
      environment: "server",
  ...
  it("requires an explicit task boundary for opaque imported calls", () => {
    ...
    `, { filename: "C:/src/Opaque.tsx" })).toThrow("task placement depends on an opaque call");
  });
  ...
  it("does not silently neutralize unresolved dynamic dispatch", () => {
    ...
    `, { filename: "C:/src/dynamic.tsx" })).toThrow("task placement depends on an opaque call");
  });
  ...
  it("keeps unknown calls visible when a known effect already restricts placement", () => {
    ...
    expect(manifest.components[0]!.tasks[0]).toMatchObject({ placement: "server", environmentEffect: "unknown" });
    expect(new Set(manifest.components[0]!.tasks[0]!.effectSources?.map(source => source.environment))).toEqual(new Set(["server", "unknown"]));
  });
  ...
  it("keeps opaque component setup visibly unknown", () => {
    ...
    expect(manifest.components[0]).toMatchObject({ placement: "unknown", environmentEffect: "unknown", artifactTargets: [] });
    expect(manifest.components[0]!.diagnostics.join("\n")).toContain("component placement depends on an opaque call");
  });

  it("resolves namespace imports and re-export chains through v2 summaries", () => {
    const provider = analyzeSource(`export function quote() { return process.env.RATE; }`, { filename: "C:/src/provider.ts" });
    ...
    expect(manifest.components[0]!.tasks[0]!.effectSources).toContainEqual(expect.objectContaining({
      environment: "server",
  ...
  it("uses declaration identity for shadowed platform globals and exports", () => {
    ...
    expect(manifest.callables.find(callable => callable.exportNames.includes("value"))?.effect).toBe("neutral");
    expect(manifest.callables.filter(callable => callable.exportNames.length).flatMap(callable => callable.exportNames)).toEqual(["value"]);
  });
  ...
  it("keeps exported value targets consistent with emitted artifacts", () => {
    ...
    expect(manifest.exports).toEqual(expect.arrayContaining([
    ...
    expect(transform(source, { filename: "C:/src/values.ts", target: "client" })).not.toContain("secret");
    expect(transform(source, { filename: "C:/src/values.ts", target: "client" })).toContain("universal");
  });
  ...
  it("maps state effects through helper parameters and preserves unknown receiver flow", () => {
    ...
    expect(exact.components[0]!.tasks[0]!.writes).toContainEqual(expect.objectContaining({
      path: "value", confidence: "exact", receiver: { kind: "component" }
    }));
    ...
    expect(broad.components[0]!.tasks[0]!.writes).toContainEqual(expect.objectContaining({
      path: "value", confidence: "unknown", receiver: { kind: "unknown" }
    }));
  });
  ...
  it("splits independent executable module initializers without changing their order", () => {
    ...
    expect(manifest.callables.filter(callable => callable.kind === "module-initializer").map(callable => callable.effect)).toEqual(["browser", "server"]);
  ...
  it("resolves side-effect-only imports through module initializer summaries", () => {
    const provider = analyzeSource(`process.stdout.write("provider");`, { filename: "C:/src/provider.ts" });
    const source = `import "./provider.js"; export function Pure() { return () => <p />; }`;
    const manifest = analyzeSource(source, { filename: "C:/src/Page.tsx", importedManifests: [provider] });
    expect(manifest.callables.find(callable => callable.kind === "module-initializer")?.effect).toBe("server");
    expect(transform(source, { filename: "C:/src/Page.tsx", target: "client", importedManifests: [provider] })).not.toContain("provider.js");
    expect(transform(source, { filename: "C:/src/Page.tsx", target: "server", importedManifests: [provider] })).toContain("provider.js");
  });

  it("rejects opaque side-effect imports instead of assuming they are neutral", () => {
    expect(() => transform(`import "opaque-package/register"; export function Pure() { return () => <p />; }`, {
      filename: "C:/src/Page.tsx"
  ...
  it("follows non-artifact TypeScript dependencies during project compilation", async () => {
    ...
    await writeFile(path.join(root, "provider.ts"), `export function quote() { return process.env.RATE; }`);
  ...
  it("invalidates artifact entries when a transitive analysis dependency changes", async () => {
    ...
    await writeFile(page, `import { value } from "./provider.js"; export function Page(this: Component<{ value?: string }>) { this.task(() => { this.state.value = value(); }); return () => <p />; }`);
    const artifactOptions = { rootDir: root, outDir, packageRoot: root, sourceRoot: root, serverComponents: true };
    ...
    expect(updated.diff.changed.map(entry => path.resolve(entry.inputFile))).toEqual([path.resolve(page)]);
