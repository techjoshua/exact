...
describe("asset imports and explicit import placement", () => {
  it("defaults side-effect stylesheet imports to client evaluation and delivery", () => {
    ...
    expect(transform(source, { filename: "C:/src/app.ts", target: "client" })).toContain("./app.scss");
    expect(transform(source, { filename: "C:/src/app.ts", target: "server" })).not.toContain("./app.scss");
  ...
  it("keeps value-bearing style modules isomorphic while delivering them to the client", () => {
    ...
    expect(transform(source, { filename: "C:/src/app.ts", target: "server" })).toContain("./app.css");
  });
  ...
  it("consumes exact import attributes and lets them override placement", () => {
    ...
    expect(transform(source, { filename: "C:/src/app.ts", target: "client" })).not.toContain("print.css");
