...
describe("exact-reactc", { timeout: 15_000 }, () => {
  it("uses the shared registry for ahead-of-time compilation", async () => {
    ...
    await writeFile(input, '/** @jsxImportSource @exact/jsx */\nimport { QueryClientProvider } from "@tanstack/react-query";\nconst view = <QueryClientProvider client={client} />;');
    await execFileAsync(process.execPath, [cli, "--compatibilityRoot", fixtureRoot, "--reactTarget", "18", "--rootDir", path.dirname(input), "--outDir", outDir, input]);
    expect(await readFile(path.join(outDir, "view.ts"), "utf8")).toContain('from "@exact/tanstack-query/react"');
  });
});
