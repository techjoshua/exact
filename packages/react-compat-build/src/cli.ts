...
async function main(argv: string[]): Promise<void> {
  ...
  const transformModule = (input: Readonly<{ id: string; source: string; target: TransformTarget }>) => {
    ...
    for (const diagnostic of result.diagnostics) if (diagnostic.severity === "warning") console.error(`[${diagnostic.code}] ${diagnostic.message} (${diagnostic.moduleId})`);
    return result;
  };
  ...
  if (!options.outDir && results.length > 1) throw new Error("exact-reactc requires --outDir when compiling more than one file");
  for (const result of results) {
...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    else if (arg === "--compatibilityRoot") options.compatibilityRoot = requiredValue(argv, ++index, arg);
    else if (arg === "--reactTarget") options.reactTarget = parseReactTarget(requiredValue(argv, ++index, arg));
...
function printUsage(): void {
  console.log("Usage: exact-reactc [exactc options] [--compatibilityRoot dir] [--reactTarget auto|18|19] <file-or-directory...>");
}
