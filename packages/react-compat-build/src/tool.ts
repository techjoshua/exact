...
import { validateReactCompatAdapterPackage } from '@exactjs/react-compat/plugin';
...
function main(argv: string[]): void {
	...
	if (command === 'validate') {
		...
		console.log(
			`${registry.replacements.size} substitution${registry.replacements.size === 1 ? '' : 's'}`
		);
		return;
	}
	if (command === 'report') {
	...
	console.log(
		'Usage: exact-react-compat validate [adapter-root] | report [build-root] [--react18|--react19]'
	);
	process.exitCode = 1;
}
