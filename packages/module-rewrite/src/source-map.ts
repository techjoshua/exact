...
export function createLineSourceMap(
	...
	for (const token of generatedTokens) {
		...
		for (let index = sourceCursor; index < limit; index++) {
			if (originals[index]!.kind === token.kind && originals[index]!.text === token.text) {
				match = index;
				break;
			}
		}
		...
		line.push({
			column: generatedLocation.character,
			sourceLine: originalLocation.line,
			sourceColumn: originalLocation.character
		});
	}
	return {
		version: 3,
...
function encodeMappings(
	...
	for (let line = 0; line < generatedLines; line++) {
		...
		const encoded = (segments.get(line) ?? [])
			.map((segment) => {
				const value =
