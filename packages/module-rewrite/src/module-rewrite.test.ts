...
import { rewriteModuleReferences } from './index.js';
...
describe('expressions module reference rewriting', () => {
	...
	it('rewrites namespace uses by binding identity without touching shadowed names', () => {
		const result = rewriteModuleReferences(
			'import * as Query from "@tanstack/react-query"; Query.QueryClientProvider;',
			{
				replacements: [
					provider,
					{
		...
		const shadowed = rewriteModuleReferences(
			'import * as Query from "@tanstack/react-query"; function f(Query: any) { return Query.QueryClientProvider; }',
			{ replacements: [provider] }
		);
	...
	it('splits CommonJS destructuring without retaining fully substituted source modules', () => {
		const result = rewriteModuleReferences(
			'const { QueryClientProvider: Provider, useQuery } = require("@tanstack/react-query");',
			{ replacements: [provider] }
		);
		expect(result.code).toContain(
