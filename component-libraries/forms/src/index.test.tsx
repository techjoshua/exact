...
describe("forms", () => {
  it("wires accessible fields and validates submission", async () => {
    ...
    render(<Form onValidSubmit={submitted}><Field name="email" validate={value => String(value).includes("@") || "Enter an email"}><Label>Email</Label><Input /><FieldError /></Field></Form>, container);
    ...
    await vi.waitFor(() => expect(container.querySelector("[role=alert]")?.textContent).toBe("Enter an email"));
  ...
  it("supports native required validation and described help", async () => {
    const submitted = vi.fn();
    const container = document.createElement("div");
    render(<Form onValidSubmit={submitted}><Field name="title" required><Label>Title</Label><Input /><span id="outside">Outside</span><FieldError /></Field></Form>, container);
    const input = container.querySelector("input")!;
  ...
  it("merges consumer accessibility attributes and assigns distinct help IDs", async () => {
    function AccessibleForm() {
      return () => <Form><Field name="email"><Input aria-describedby="consumer" aria-invalid="grammar" /><FieldHelp>Primary help</FieldHelp><FieldHelp>Secondary help</FieldHelp></Field></Form>;
    }
    const view = await testComponent(AccessibleForm).mount();
    const input = view.getByRole("textbox").element;
    expect(input.getAttribute("aria-describedby")).toBe("consumer exact-field-email-help exact-field-email-help-2");
  ...
  it("awaits asynchronous validation and submission through testing actions", async () => {
    ...
    const submitted = vi.fn();
    function AsyncForm() {
      return () => <Form onValidSubmit={submitted}><Field name="name" validate={async () => { await gate; }}><Input /></Field><button type="submit">Save</button></Form>;
    }
    const view = await testComponent(AsyncForm).mount();
    let settled = false;
    const action = view.getByRole("button", { name: "Save" }).submit().then(() => { settled = true; });
  ...
  it("coordinates native constraints for real requestSubmit actions", async () => {
    ...
    function RequiredForm() {
      return () => <Form onValidSubmit={submitted}><Field name="name" required><Input /><FieldError /></Field><button type="submit">Save</button></Form>;
    }
  ...
  it("rejects duplicate generated field IDs while allowing explicit distinct IDs", async () => {
    ...
    function DuplicateForm() {
      return () => <Form><Field name="choice"><Input /></Field><Field name="choice"><Input /></Field></Form>;
    }
    const duplicate = await testComponent(DuplicateForm).context(ErrorContext, errors).mount();
    expect(errors.errors[0]?.error).toEqual(expect.objectContaining({ message: expect.stringContaining("explicit distinct ids") }));
    expect(duplicate.getAllByRole("textbox")).toHaveLength(1);
    duplicate.unmount();

    function DistinctForm() {
      return () => <Form><Field name="choice" id="choice-a"><Input /></Field><Field name="choice" id="choice-b"><Input /></Field></Form>;
    }
