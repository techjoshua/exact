...
export function Submit(this: Component<{}>, props: SubmitProps) {
	...
	return () => (
		...
		>
			{form.submitting && pendingText !== undefined ? pendingText : children}
		</button>
	);
}
...
function controlComponent(
	...
	const combined: RefBinding<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement> = {
		get current() {
	...
	return () =>
		tag === 'textarea' ? (
			<textarea {...controlProps()} ref={combined as RefBinding<HTMLTextAreaElement>}>
				{children}
			</textarea>
		) : tag === 'select' ? (
			<select {...controlProps()} ref={combined as RefBinding<HTMLSelectElement>}>
				{children}
			</select>
		) : (
			<input {...controlProps()} ref={combined as RefBinding<HTMLInputElement>} />
		);
}
