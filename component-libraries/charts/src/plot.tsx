...
export function ChartPlot(this: Component<ChartPlotState>) {
	...
	const inspect = (event: Event) => selectChartMark(this.state, event);
	...
	return () => (
		<>
			...
			>
				...
				<g className="exact-chart__ticks" aria-hidden="true">
					{presentation.ticks.map((tick) => (
						<text
							key={tick.id}
							x={presentation.orientation === 'horizontal' ? tick.x : presentation.layout.left - 8}
							y={
				...
				<g className="exact-chart__axis-labels" aria-hidden="true">
					{presentation.axisLabels.map((label) => (
						<text
							key={label.id}
							x={axisLabelX(label.position, presentation.layout)}
							y={axisLabelY(label.position, presentation.layout)}
							text-anchor="middle"
							transform={
								label.position === 'left' || label.position === 'right'
									? `rotate(-90 ${axisLabelX(label.position, presentation.layout)} ${axisLabelY(label.position, presentation.layout)})`
									: undefined
							}
						>
							{label.value}
				...
				{presentation.lines.map((series) => (
					...
					>
						{series.area && (
							<path d={series.area} fill="var(--exact-chart-series-current)" fill-opacity="0.18" />
						)}
						<path
							d={series.path}
			...
			<div
				id={`${chart.id}-tooltip`}
				className="exact-chart__tooltip"
				role="tooltip"
				hidden={!active}
				...
				style={
					active
						? `inset-inline-start:${clampPercent((active.x / presentation.layout.width) * 100)}%;inset-block-start:${clampPercent((active.y / presentation.layout.height) * 100)}%`
						: undefined
				}
			>
