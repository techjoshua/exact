...
const RouteErrorContext = createContext<Readonly<{ active: boolean; error?: unknown }>>({
	active: false
});
const RouteSnapshotOverrideContext = createContext<ExactRouterSnapshot<RouteObject> | undefined>(
	undefined
);
...
function renderHydrationFallback(snapshot: ExactRouterSnapshot<RouteObject>): {
	found: boolean;
	node?: ReactNode;
} {
	let fallbackIndex = -1;
