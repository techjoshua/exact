...
describe('React Router modern facade', () => {
	...
	it('runs data loaders through RouterProvider', async () => {
		...
		const container = document.createElement('div');
		createRoot(container).render(
			createElement(RouterProvider, { router, fallbackElement: 'Loading' })
		);
		await settle();
		await settle();
	...
	it('runs static loaders with request context and returns redirect responses', async () => {
		...
		const handler = createStaticHandler([
			...
			},
			{
				id: 'old',
				path: 'old',
				loader: () => new Response(null, { status: 307, headers: { Location: '/home' } })
			}
		]);
	...
	it('preserves static loader response status and headers and follows thrown action redirects', async () => {
		const handler = createStaticHandler([
			{
				...
				action: () => {
					throw new Response(null, { status: 303, headers: { Location: '/done' } });
				}
			},
			{ id: 'done', path: 'done' }
		]);
		...
		expect(
			(context as Exclude<typeof context, Response>).loaderHeaders.report?.get('X-Route')
		).toBe('report');
		const redirect = await handler.query(
			new Request('https://example.test/report', { method: 'POST' })
		);
	...
	it('rejects hydration for another location and supports keyed router roots', async () => {
		...
		const adopted = createBrowserRouter([{ id: 'home', path: 'home', loader: () => 'client' }], {
	...
	it('observes external history changes and releases its subscriptions', async () => {
		const listeners = new Set<() => void>();
		const history = {
			location: '/one',
			action: 'POP' as 'POP' | 'PUSH' | 'REPLACE',
			push(to: unknown) {
				this.location = String(to);
				this.action = 'PUSH';
	...
	it('leaves cross-origin links to normal browser navigation', async () => {
		...
		container
			.querySelector('a')!
			.onclick?.call(container.querySelector('a')!, event as PointerEvent);
		expect(event.defaultPrevented).toBe(false);
	});

	it('preserves ancestor layouts and exposes descendant loader errors to the selected boundary', async () => {
		function Layout() {
			return createElement('main', null, 'Layout:', createElement(Outlet, {}));
		}
		function Boundary() {
	...
	it('renders route hydration fallbacks inside ancestor layouts', async () => {
		...
		const container = document.createElement('div');
		createRoot(container).render(
			createElement(RouterProvider, { router, fallbackElement: 'Global loading' })
		);
		await settle();
	...
	it('materializes renderer fields from lazy routes', async () => {
		function LazyView() {
			return createElement('p', null, 'Lazy ready');
		}
		const router = createMemoryRouter(
			[
				{
	...
	it('settles Await promises and exposes their resolved value', async () => {
		...
		createRoot(container).render(
			createElement(
				...
				createElement(Await, {
					resolve: pending,
					children: (value: unknown) => `Resolved ${value}`
				})
			)
		);
		await settle();
