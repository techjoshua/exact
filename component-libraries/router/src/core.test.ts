...
describe('renderer-neutral router core', () => {
	...
	it('matches absolute child paths from the route root', () => {
		...
		expect(matchRoutes(absolute, '/app/settings').map((match) => match.id)).toEqual([
			'app',
			'settings'
		]);
	});
	...
	it('provides standalone matching and path generation', () => {
		expect(
			matchPath({ path: '/teams/:teamId', end: false }, '/teams/exact/members')?.params
	...
	it('publishes immutable snapshots and one notification per transition stage', async () => {
		const source = createMemoryLocationSource(
			['https://example.test/', 'https://example.test/users/1'],
			0
		);
		...
		const dataRoutes = [
			{
				...
				children: [
					...
					},
					{
						id: 'lazy',
						path: 'lazy',
						lazy: async () => ({ loader: () => 'lazy data', handle: 'ready' })
					}
				]
			}
		] satisfies readonly ExactRouteDefinition[];
	...
	it('validates bounded JSON-safe route hydration data', async () => {
		...
		expect(() => hydrationDataFromSnapshot(safe.getSnapshot(), { maxBytes: 1 })).toThrow(
			/byte limits/
		);
			...
			routes: [
				{
					...
					children: [
						{ id: 'loop', path: 'loop', loader: () => redirect('/loop') },
						{
							id: 'item',
							path: 'items/:id',
							loader: ({ params }: any) => (params.id === '1' ? first : params.id)
						}
					]
				}
			]
