...
describe("router compatibility integration", () => {
  it("selects data and v5 facades from distinct resolved package instances", () => {
    ...
    const routerManifest = JSON.parse(readFileSync(new URL("../package.json", import.meta.url), "utf8"));
    ...
    const modern = engine.transformModule({
      ...
      source: 'import { RouterProvider, useNavigate } from "react-router-dom"; export { RouterProvider, useNavigate };',
      format: "module",
      target: "client"
    });
    const legacy = engine.transformModule({
      id: path.join(root, "node_modules/nested/src/view.ts"),
      source: 'import { Switch, useHistory } from "react-router-dom"; export { Switch, useHistory };',
      format: "module",
      target: "client"
    });
    expect(modern.code).toContain('from "@exact/router/data"');
    expect(legacy.code).toContain('from "@exact/router/v5"');
    expect(engine.report().substitutions).toEqual(expect.arrayContaining([
