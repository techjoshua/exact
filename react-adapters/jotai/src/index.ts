...
export function ExactJotaiProvider(this: Component<Record<string, unknown>>, props: ExactJotaiProviderProps) {
  this.setContext(JotaiStoreContext, props.store ?? getDefaultStore());
  return () => props.children ?? null;
}

export function createAtomSource<Value>(store: Store, valueAtom: Atom<Value>): ExternalSource<Value> {
...
export function createComponentAtom<Value>(component: Component<any>, valueAtom: Atom<Value>, store?: Store): ExternalSource<Value> {
  let resolved = store;
  if (!resolved) {
    try { resolved = component.getContext(JotaiStoreContext); }
    catch { resolved = getDefaultStore(); }
  }
