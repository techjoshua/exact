...
import { ExactJotaiProvider, JotaiStoreContext, type ExactJotaiProviderProps, type Store } from "./index.js";

export const Provider: ReactComponentType<ExactJotaiProviderProps> = exposeExactComponent(ExactJotaiProvider, "JotaiProvider");

export function useStore(options?: Readonly<{ store?: Store }>): Store {
  if (options?.store) return options.store;
  try { return useExactContext(JotaiStoreContext); }
  catch { return getDefaultStore(); }
}

export function useAtomValue<Value>(valueAtom: Atom<Value>, options?: Readonly<{ store?: Store }>): Value {
  const store = useStore(options);
  return useSyncExternalStore(callback => store.sub(valueAtom, callback), () => store.get(valueAtom), () => store.get(valueAtom));
}
