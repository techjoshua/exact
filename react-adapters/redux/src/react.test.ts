...
describe("@exact/redux React bridge", () => {
  it("provides React Redux-compatible custom context without a react-redux runtime import", async () => {
    ...
    let observed: any;
    function Reader() { observed = useContext(CustomContext); return null; }
    const container = document.createElement("div");
    createRoot(container).render(createElement(Provider, { store, context: CustomContext }, createElement(Reader, null)));
