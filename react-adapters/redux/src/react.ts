...
import { createElement, useEffect, useMemo, useSyncExternalStore, type ReactComponentType, type ReactContext, type ReactNode } from "@exact/react-compat";
import { unwrap } from "@exact/reactive";
import { createReduxSubscription, ExactReduxProvider, ReduxServerStateContext, ReduxStoreContext, type ExactReduxProviderProps, type ReduxStore } from "./index.js";

const NativeProvider = exposeExactComponent(ExactReduxProvider, "ReduxProvider");
export const Provider: ReactComponentType<ExactReduxProviderProps> = function ReduxProvider(props): ReactNode {
...
export function useDispatch<Action = unknown>(): ReduxStore<unknown, Action>["dispatch"] { return useStore<unknown, Action>().dispatch; }
export function useSelector<State, Selected>(selector: (state: State) => Selected, equality: (left: Selected, right: Selected) => boolean = Object.is): Selected {
