...
- Use partial `theme:typography` objects for custom fonts. Omitted source fields and background policies inherit. Use `inverse` for the opposite parent appearance or `inverse-system` for the opposite browser preference; read `ThemeContext.appearance` only when known.
