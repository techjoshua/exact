...
type jsxRuntimeNames struct {
	...
	textHostElement           string
	intrinsicComposition      string
...
const (
	...
	runtimeTextHost                   runtimeImportGroupID = "text-host"
...
func (lowering *jsxLowering) runtimeImports(root *ast.Node) []*ast.Node {
	...
	groups := []runtimeImportGroup{
		...
		{id: runtimeTextHost, module: "@exactjs/dom/runtime/text-host"},
	...
	helpers := []runtimeImportHelper{
		...
		{"createCompiledTextHostReceipt", lowering.names.textHostElement, runtimeTextHost},
		{"withIntrinsicComposition", lowering.names.intrinsicComposition, runtimeComponentABI},
		...
		{"createCompiledDocumentReceipt", lowering.names.documentReceipt, runtimeComponentABI},
		...
		{"createCompiledSuppliedTargetReceipt", lowering.names.suppliedTarget, runtimeComponentABI},
		...
		{"compiledReactivePropertyOperand", lowering.names.propertyOperand, runtimeReactivity},
	...
	targetUsed := lowering.target != TargetServer &&
		(containsIdentifier(root, lowering.names.target) || containsIdentifier(root, lowering.names.suppliedTarget) ||
...
func isCompiledTargetIssuer(reference externalImportReference) bool {
	return ((reference.moduleSpecifier == "@exactjs/core/runtime/component-abi" ||
		...
		(reference.exportName == "createCompiledTargetReceipt" || reference.exportName == "createCompiledSuppliedTargetReceipt" ||
			reference.exportName == "createCompiledTargetContributions" ||
			reference.exportName == "createCompiledFragmentPresentation")) ||
...
func allocateJSXRuntimeNames(sourceFile *ast.SourceFile) jsxRuntimeNames {
	...
	return jsxRuntimeNames{
		...
		textHostElement:           allocate("__exactTextHostReceipt"),
		intrinsicComposition:      allocate("__exactComposableIntrinsic"),
		...
		documentReceipt:           allocate("__exactDocumentReceipt"),
		...
		suppliedTarget:            allocate("__exactSuppliedTarget"),
		suppliedProps:             allocate("__exactSuppliedProps"),
		...
		propertyOperand:           allocate("__exactPropertyOperand"),
