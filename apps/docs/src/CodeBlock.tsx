...
type CodeBlockProps = {
	source: string;
	language?: 'tsx' | 'ts' | 'json' | 'css' | 'shell' | 'logo';
	title?: string;
	...
	return () => (
		<figure className={props.compact ? 'code-block code-block--compact' : 'code-block'}>
			...
			<pre tabindex="0" aria-label={`${props.title ?? 'Code'} in ${language}`}>
				<code>
					{lines.map((line) => (
						<span className={highlighted.has(line.number) ? 'code-line is-highlighted' : 'code-line'}>
...
function tokenize(source: string, language: CodeBlockProps['language']): CodeLine[] {
	return source.split('\n').map((line, lineIndex) => ({
		id: `line-${lineIndex}`,
		number: lineIndex + 1,
...
function tokenizeLine(
	...
	const expression = /(\/\/.*|\/\*.*?\*\/|<!--[\s\S]*?-->|"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|`(?:\\.|[^`\\])*`|\b\d+(?:\.\d+)?\b|[A-Za-z_$][\w$]*|<\/?[A-Za-z][\w.-]*|\/?\>|=>|===|!==|==|!=|<=|>=|&&|\|\||\+\+|--|\*\*|[{}()[\].,:;?+\-*/%=<>!&|])/g;
	let cursor = 0;
...
function tokenizeShell(line: string, lineIndex: number): CodeToken[] {
	const parts = line.match(/#.*|"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|--?[\w-]+|\b(?:npm|npx|node|git)\b|\s+|\S+/g) ?? [''];
	return parts.map((text, index) => {
