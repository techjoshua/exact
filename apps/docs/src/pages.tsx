	...
	return () => (
		...
		>
			<Callout title="Repository-first setup" tone="tip">
				<p>
					The packages are currently developed together in this repository. These examples use
					workspace dependencies rather than pretending a public package-install story is already
					settled.
				</p>
			</Callout>
			...
			<section>
				...
				<p>
					Create a workspace application and connect the small set of packages needed for browser
					rendering.
				</p>
			...
			<section>
				...
				<p>
					The JSX package provides TypeScript’s runtime entrypoints and element types. The compiler
					plugin turns those expressions into eXact’s fine-grained boundaries.
				</p>
			...
			<section>
				...
				<p>
					Uncompiled JSX can describe a static tree, but JavaScript has already reduced expressions
					to plain values. The eXact compiler preserves each expression as a reactive cell so a text
					value, property, style, or child can update at its own boundary.
				</p>
			</section>
		</Article>
	);
	...
	return () => (
		...
		>
			<section>
				...
				<p>
					A list is not only an array of rendered strings. Each row may own input selection, focus,
			...
			<section>
				...
				<p>
					The framework owns the JSX key. Duplicate keys fail deterministically rather than falling
					back to position and risking state corruption. String arrays use the string value as their
					key.
				</p>
			</section>
			<section>
	...
	return () => (
		...
		>
			...
			<section>
				...
				<p>
					Expressions before the task callback are dependencies. The compiler recognizes direct
					state and prop expressions and wraps them as reactive values. When a dependency changes,
					eXact aborts the current generation, waits for registered cleanup as needed, and invokes
					the callback with current unwrapped values.
				</p>
				<CodeBlock source={taskSource} language="tsx" title="Search.tsx" />
				<p>
					For the search above, changing <code>query</code> aborts the older fetch before a new
					request begins. The same generation signal aborts when the component unmounts, preventing
					a stale response from continuing as live component work.
				</p>
			</section>
			<section>
	...
	return () => (
		...
		>
			<section>
				...
				<p>
					A controlled input normally repeats the same state path twice: once to project state into
					a DOM property and once to copy the browser's next value back during an event. eXact
					supports a narrow
			...
			<section>
				...
				<p>
					Labels remain labels, inputs remain inputs, and the browser still participates in
					validation. Form context coordinates the accessible relationships rather than replacing
					them with a proprietary field model.
				</p>
			</section>
		</Article>
	);
