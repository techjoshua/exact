...
describe("Census map paths", () => {
  it("includes the states, DC, and Puerto Rico without projection spikes", () => {
    ...
    expect(usStatePaths.map(state => state.abbreviation)).toEqual(expect.arrayContaining(["CA", "AK", "HI", "DC", "PR"]));
    for (const state of usStatePaths) {
      const points = [...state.d.matchAll(/[ML](-?[\d.]+) (-?[\d.]+)/g)].map(match => [Number(match[1]), Number(match[2])]);
