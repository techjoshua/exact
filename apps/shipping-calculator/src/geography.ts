...
const points = new Map((centroids as unknown as [string, number, number][]).map(([zip, latitude, longitude]) => [zip, { zip, latitude, longitude }]));

export function pointForZip(zip: string): GeoPoint | undefined { return points.get(zip.slice(0, 5)); }
...
export function resolveRoute(originZip: string, destinationZip: string): RouteResult {
  ...
  return { status: "success", origin, destination, distanceMiles: Math.round(haversineMiles(origin, destination)) };
}
