...
export type ShipmentDraft = {
  ...
  kind: MailpieceKind;
  preset: "custom" | "mailer" | "small-box" | "medium-box" | "large-box" | "letter" | "large-envelope";
  pounds: string;
  ounces: string;
  length: string;
  width: string;
...
export type RateQuote = {
  ...
  delivery: { minimumDays?: number; maximumDays?: number; estimatedDate?: string; guaranteed: boolean };
  charges: Charge[];
...
export type PublicProviderError = {
  code: "not_configured" | "invalid_request" | "unauthorized" | "rate_limited" | "unavailable" | "timeout";
  message: string;
