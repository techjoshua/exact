...
describe("DOOP reference rates", () => {
  ...
  it("prices paid tracking on Scoot and included tracking on Standard", async () => {
    ...
    expect(scoot.features.find(feature => feature.code === "tracking")?.availability).toBe("available");
    expect(scoot.charges.find(charge => charge.code === "tracking")?.amountCents).toBe(79);
    expect(standard.features.find(feature => feature.code === "tracking")?.availability).toBe("included");
  });

  it("uses dimensional weight and adds oversize, signature, and insurance charges", async () => {
    const request = normalizeDraft({ ...defaultDraft, length: "70", width: "24", height: "24", declaredValue: "500", insurance: true, signature: "adult" });
    ...
    expect(scoot.charges.map(charge => charge.code)).toEqual(expect.arrayContaining(["oversize", "insurance"]));
    expect(scoot.charges.map(charge => charge.code)).not.toContain("signature");
    expect(scoot.totalPriceCents).toBe(scoot.charges.reduce((sum, charge) => sum + charge.amountCents, 0));
    expect(scoot.compatible).toBe(false);
    expect(scoot.features.find(feature => feature.code === "adult-signature")?.availability).toBe("unavailable");
  });
});
