...
const fixture = async (name: string) =>
	JSON.parse(await readFile(new URL(`./fixtures/${name}-rates.json`, import.meta.url), 'utf8'));
...
describe('live carrier adapters', () => {
	it.each([
		[
			...
			createUspsProvider,
			{
				...
				USPS_RATE_URL: 'https://fixture.test/rate'
			}
		],
		[
			'ups',
			createUpsProvider,
			{
				...
				UPS_RATE_URL: 'https://fixture.test/rate'
			}
		],
		[
			'fedex',
			createFedexProvider,
			{
				...
				FEDEX_RATE_URL: 'https://fixture.test/rate'
			}
		],
		[
			'dhl',
			createDhlProvider,
			{
				...
				DHL_RATE_URL: 'https://fixture.test/rate'
			}
		]
		...
		const fetchMock = async (
			...
			if (url.includes('oauth') || url.includes('token'))
				return Response.json({ access_token: `token-${id}`, expires_in: 3600 });
		...
		expect(quotes[0]!.totalPriceCents).toBe(
			quotes[0]!.charges.reduce((sum, charge) => sum + charge.amountCents, 0)
		);
	...
	it('surfaces HTTP status and Retry-After without response contents', async () => {
		const provider = createDhlProvider({
			DHL_API_KEY: 'id',
			DHL_API_SECRET: 'secret',
			DHL_ACCOUNT_NUMBER: 'account',
			DHL_RATE_URL: 'https://fixture.test/rate'
		...
		await expect(
			provider.quote(request, {
				signal: new AbortController().signal,
				fetch: (async () =>
					new Response('account data', {
						status: 429,
						headers: { 'retry-after': '12' }
					})) as typeof fetch
			})
		).rejects.toEqual(
			expect.objectContaining<Partial<ProviderHttpError>>({ status: 429, retryAfterSeconds: 12 })
		);
	});
});
