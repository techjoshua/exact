...
describe('shipment model', () => {
	...
	it('rejects malformed ZIPs, zero weight, non-finite dimensions, and insurance without value', () => {
		...
		expect(() => normalizeDraft({ ...defaultDraft, declaredValue: '', insurance: true })).toThrow(
			/declared value/
		);
	});
	...
	it('round-trips readable URL state in stable order', () => {
		const draft = {
			...defaultDraft,
			signature: 'adult' as const,
			declaredValue: '250',
			insurance: true,
			machinable: false
		};
		const url = draftUrl(draft, new URL('https://example.test/calculator'));
		expect([...url.searchParams.keys()].slice(0, 5)).toEqual([
			'from',
			'to',
			'kind',
			'weight',
	...
	it('always ranks compatible quotes above incompatible quotes', () => {
		const quote = (id: string, cents: number, compatible: boolean, days: number): RateQuote => ({
			...
			delivery: { maximumDays: days, guaranteed: false },
			charges: [],
			features: [],
			compatible,
			warnings: [],
		...
		expect(
			rankQuotes([quote('cheap-bad', 1, false, 1), quote('good', 500, true, 5)], 'cheapest').map(
				(item) => item.id
			)
		).toEqual(['good', 'cheap-bad']);
	});
});
