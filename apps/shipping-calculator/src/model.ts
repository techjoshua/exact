...
export function normalizeDraft(draft: ShipmentDraft): RateRequest {
	...
	if (!zipPattern.test(originZip) || !zipPattern.test(destinationZip))
		throw new Error('Enter valid 5-digit or ZIP+4 codes');
	...
	const value = draft.declaredValue.trim()
		? finite(draft.declaredValue, 'declared value', 0, 50_000)
		: undefined;
	if (draft.insurance && value === undefined)
		throw new Error('Enter a declared value to price insurance');
...
export function parseRateRequest(value: unknown): RateRequest {
	...
	const draft: ShipmentDraft = {
		...
		declaredValue:
			record.declaredValueCents === undefined
				? ''
				: String(number(record.declaredValueCents) / 100),
...
export function draftFromUrl(url: URL): { draft: ShipmentDraft; explicit: boolean } {
	...
	if (['parcel', 'envelope', 'flat'].includes(params.get('kind') ?? ''))
		draft.kind = params.get('kind') as MailpieceKind;
	const weight = params.get('weight')?.match(/^(\d+(?:\.\d+)?)lb(?:-(\d+(?:\.\d+)?)oz)?$/);
	if (weight) {
		draft.pounds = weight[1]!;
		draft.ounces = weight[2] ?? '0';
	}
...
export function rankQuotes(
	...
	return [...quotes].sort((left, right) => {
		...
		if (sort === 'fastest')
			return (
				deliveryDays(left) - deliveryDays(right) || left.totalPriceCents - right.totalPriceCents
			);
...
export function emptyInitialModel(
	...
	return {
		...
		request,
		explicitUrlState,
		configuredProviders: ['doop'],
		route: { status: 'unavailable' },
		providers: []
	};
}

function deliveryDays(quote: RateQuote): number {
	return quote.delivery.maximumDays ?? quote.delivery.minimumDays ?? 999;
}
...
function string(value: unknown): string {
	if (typeof value !== 'string') throw new Error('Invalid rate request');
	return value;
}
function number(value: unknown): number {
	if (typeof value !== 'number' || !Number.isFinite(value)) throw new Error('Invalid rate request');
	return value;
}
function boolean(value: unknown): boolean {
	if (typeof value !== 'boolean') throw new Error('Invalid rate request');
	return value;
}
