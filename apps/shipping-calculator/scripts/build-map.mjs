...
const states = shapes.flatMap((shape, index) => {
  const record = records[index];
  if (!record || excluded.has(record.STATEFP)) return [];
  return [{ fips: record.STATEFP, abbreviation: record.STUSPS, name: record.NAME, d: shapePath(shape, record.STATEFP) }];
}).sort((left, right) => left.fips.localeCompare(right.fips));

const source = `/** Generated from the U.S. Census Bureau 2025 1:20m state cartographic boundaries. */\n` +
...
function readDbf(buffer) {
  ...
  for (let offset = 32; buffer[offset] !== 0x0d; offset += 32) {
    ...
    fields.push({ name: buffer.toString("ascii", offset, end < 0 ? offset + 11 : end), length: buffer[offset + 16] });
  }
...
function readShapes(buffer) {
  ...
  for (let offset = 100; offset + 8 <= buffer.length;) {
    ...
    if (type === 5) {
      ...
      const points = Array.from({ length: pointCount }, (_, index) => [buffer.readDoubleLE(pointsStart + index * 16), buffer.readDoubleLE(pointsStart + index * 16 + 8)]);
      shapes.push(parts.map((first, index) => points.slice(first, parts[index + 1] ?? points.length)));
