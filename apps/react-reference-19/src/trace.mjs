...
const result = process.argv.includes("--phase6")
  ? await collectReactPhase6Trace({ React, ReactDOM, ReactDOMServer, baseline: "19.2" })
  : process.argv.includes("--phase5")
  ? await collectReactPhase5Trace({ React, ReactDOMClient, ReactDOMServer, JSDOM, baseline: "19.2" })
  : process.argv.includes("--phase4")
  ? await collectReactPhase4Trace({ React, ReactDOMClient, JSDOM, baseline: "19.2" })
  : process.argv.includes("--phase3")
  ? await collectReactPhase3Trace({ React, ReactDOM, ReactDOMClient, JSDOM, baseline: "19.2" })
  : process.argv.includes("--phase2")
  ? await collectReactPhase2Trace({ React, ReactDOMClient, JSDOM, baseline: "19.2" })
  : process.argv.includes("--phase1")
  ? await collectReactPhase1Trace({ React, ReactDOMClient, JSDOM, baseline: "19.2" })
  : process.argv.includes("--benchmark")
  ? benchmarkReactReference({ React, ReactDOMServer, baseline: "19.2" })
  : await collectReactReferenceTrace({ React, ReactDOM, ReactDOMClient, ReactDOMServer, ReactJsxRuntime, ReactJsxDevRuntime, ReactCompilerRuntime, JSDOM, baseline: "19.2" });
