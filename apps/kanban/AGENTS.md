...
`BoardContext`. Persistence is a client-placed reactive task whose serialized argument is its
dependency. Reactive activation already supersedes prior work, so do not add manual generation
state or unnecessary invoked-task concurrency policy.
