...
export function seedTasks(): Task[] {
	return [
		createTask(
			'Shape the v0 public API contract',
			'Define what is stable enough for external examples.',
			'active',
			'high',
			'Mara',
			['api', 'docs']
		...
		createTask(
			...
			'Use refs and context-backed services for keyboard-driven workflows.',
			'backlog',
			'medium',
			'Jo',
			['ux']
		...
		createTask(
			...
			'Exercise task lifecycles, serialization, and error reporting.',
			'review',
			'medium',
			'Ren',
			['data']
		...
		createTask(
			'Document reactive compiler edge cases',
			'Make strict equality and expression-boundary behavior explicit.',
			'done',
			'high',
			'Ari',
			['compiler', 'docs']
		...
		createTask(
			'Test focus handoff through refs',
			'Open detail and palette flows without losing editing context.',
			'active',
			'medium',
			'Mara',
			['refs']
		...
		createTask(
			'Simulate recoverable sync failures',
			'Show how errors surface without making every component a boundary.',
			'backlog',
			'low',
			'Ari',
			['errors']
...
export function parseTaskImport(source: string): Task[] {
	...
	return parsed.map((item, index) => {
		...
		return {
			...
			priority,
			owner:
				typeof candidate.owner === 'string' && candidate.owner ? candidate.owner : 'Unassigned',
