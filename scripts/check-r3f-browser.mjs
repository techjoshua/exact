...
import { releaseCacheKey, restoreCachedArtifact, storeCachedArtifact } from './release-cache.mjs';
...
try {
	...
	if (!cacheHit) {
		await build({
			entryPoints: [
				path.join(workspace, 'apps/react-reconciler-reference-19/src/browser-scenario.mjs')
			],
			...
			plugins: [
				{
					name: 'exact-r3f-browser-singleton',
					setup(buildApi) {
						buildApi.onResolve(
	...
	await writeFile(
		...
		'<!doctype html><style>html,body,#root{width:320px;height:240px;margin:0}</style><div id="root"></div><script type="module" src="/scenario.js"></script>'
	);
	...
	const runBrowser = async ([name, browserType]) => {
		...
		try {
			...
			const cyclesPassed =
				result.cycles.length === 5 && result.cycles.every((cycle) => cycle.passed);
	...
	if (results.length !== 3 || incomplete.length > 0) {
		throw new Error(
			`Every declared browser must pass the real-WebGL R3F gate: ${JSON.stringify(incomplete)}`
		);
	}
