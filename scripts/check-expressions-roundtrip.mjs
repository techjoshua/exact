...
async function checkGroup(config, filenames) {
  ...
  try {
    ...
    for (const filename of filenames) {
      ...
      if (module.emit({ format: "preserve" }).code !== module.source) throw new Error(`Lossless round trip changed ${filename}`);
    }
    return { config, fileCount: filenames.length, elapsedMs: performance.now() - started, events: collector.snapshot() };
  } finally {
    project.dispose();
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", code => {
      ...
      try {
        ...
        reject(new Error(`Expression worker returned an invalid profile for ${config}`, { cause: error }));
      }
    });
...
function printProfileSummary(projects) {
  ...
  const typeEvents = projects.flatMap(project => project.events)
  ...
  const ratio = (hits, misses) => hits + misses === 0 ? "n/a" : `${(hits / (hits + misses) * 100).toFixed(1)}%`;
  console.log("Projection query counters:");
  console.log(`  checker type queries              ${total("checkerTypeQueries").toLocaleString()}`);
  console.log(`  type cache hit rate               ${ratio(typeHits, typeMisses)} (${typeHits.toLocaleString()} hits / ${typeMisses.toLocaleString()} misses)`);
  console.log(`  shallow type cache hit rate       ${ratio(shallowHits, shallowMisses)} (${shallowHits.toLocaleString()} hits / ${shallowMisses.toLocaleString()} misses)`);
  const allEvents = projects.flatMap(project => project.events);
  const totalAcross = (phase, field) => allEvents
    .filter(event => event.phase === phase)
    .reduce((sum, event) => sum + (event[field] ?? 0), 0);
  console.log(`  checker symbol queries            ${totalAcross("projection-node-bindings", "checkerSymbolQueries").toLocaleString()}`);
  console.log(`  resolved signature queries        ${totalAcross("projection-node-specialization", "resolvedSignatureQueries").toLocaleString()}`);
  console.log(`  directive characters scanned      ${totalAcross("projection-node-common", "directiveCharacters").toLocaleString()}`);
}
