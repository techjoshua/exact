...
export async function collectReactReferenceTrace({ React, ReactDOM, ReactDOMClient, ReactDOMServer, ReactJsxRuntime, ReactJsxDevRuntime, ReactCompilerRuntime, JSDOM, baseline }) {
  ...
  function Counter() {
    ...
    return React.createElement("button", { type: "button", onClick: () => setCount(value => value + 1) }, String(count));
  }
  ...
  try {
    ...
    return {
      ...
      exports: {
        ...
        ...(ReactCompilerRuntime ? { "react/compiler-runtime": publicExports(ReactCompilerRuntime) } : {})
      },
      element: {
        type: element.type,
        key: element.key,
...
export function benchmarkReactReference({ React, ReactDOMServer, baseline, iterations = 200 }) {
  function Tree({ offset }) {
    return React.createElement("ul", null, Array.from({ length: 100 }, (_, index) =>
...
function publicExports(module) {
  return Object.keys(module).filter(name => name !== "default" && name !== "module.exports").sort();
}

function installDomGlobals(window) {
  const names = ["window", "document", "navigator", "Node", "Element", "HTMLElement", "Event", "MouseEvent"];
