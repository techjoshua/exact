...
} from '../packages/react-compatibility/dist/index.js';
...
async function runReference(workspace) {
	...
	const { stdout } = await execFileAsync(
		npm.file,
		[...npm.args, 'run', 'trace', '-w', workspace, '--silent'],
		{
			cwd: root,
			encoding: 'utf8',
			windowsHide: true
...
async function runWorkspaceScript(workspace, script) {
	...
	const { stdout } = await execFileAsync(
		npm.file,
		[...npm.args, 'run', script, '-w', workspace, '--silent'],
		{
			cwd: root,
			encoding: 'utf8',
			windowsHide: true
...
async function runExactPhase1(target) {
	const { stdout } = await execFileAsync(
		process.execPath,
		['scripts/run-exact-react-phase1.mjs', String(target)],
		{
			cwd: root,
			encoding: 'utf8',
			windowsHide: true
...
async function validatePhase2Compatibility() {
	...
	writeFileSync(
		path.join(outputDirectory, 'phase-2-exact-18.json'),
		`${JSON.stringify(cases[0][1], null, 2)}\n`
	);
	writeFileSync(
		path.join(outputDirectory, 'phase-2-exact-19.json'),
		`${JSON.stringify(cases[1][1], null, 2)}\n`
	);
	console.log(
		'React Phase 2 context, effect, ref, memo, and external-store traces agree with React 18 and React 19'
	);
}
...
async function validatePhase3Compatibility() {
	...
	writeFileSync(
		path.join(outputDirectory, 'phase-3-exact-18.json'),
		`${JSON.stringify(cases[0][1], null, 2)}\n`
	);
	writeFileSync(
		path.join(outputDirectory, 'phase-3-exact-19.json'),
		`${JSON.stringify(cases[1][1], null, 2)}\n`
	);
	console.log(
		'React Phase 3 portal, Suspense, lazy, deferred-value, and transition traces agree with React 18 and React 19'
	);
}
...
async function validatePhase4Compatibility() {
	...
	writeFileSync(
		path.join(outputDirectory, 'phase-4-exact-18.json'),
		`${JSON.stringify(cases[0][1], null, 2)}\n`
	);
	writeFileSync(
		path.join(outputDirectory, 'phase-4-exact-19.json'),
		`${JSON.stringify(cases[1][1], null, 2)}\n`
	);
	console.log(
		'React Phase 4 class, boundary, PureComponent, lifecycle, and Profiler traces agree with React 18 and React 19'
	);
}
...
async function validatePhase5Compatibility() {
	...
	writeFileSync(
		path.join(outputDirectory, 'phase-5-exact-18.json'),
		`${JSON.stringify(cases[0][1], null, 2)}\n`
	);
	writeFileSync(
		path.join(outputDirectory, 'phase-5-exact-19.json'),
		`${JSON.stringify(cases[1][1], null, 2)}\n`
	);
	console.log(
		'React Phase 5 server markup, pipeable stream, and hydration traces agree with React 18 and React 19'
	);
}
...
async function validatePhase6Compatibility() {
	...
	writeFileSync(
		path.join(outputDirectory, 'phase-6-exact-18.json'),
		`${JSON.stringify(cases[0][1], null, 2)}\n`
	);
	writeFileSync(
		path.join(outputDirectory, 'phase-6-exact-19.json'),
		`${JSON.stringify(cases[1][1], null, 2)}\n`
	);
	console.log(
		'React Phase 6 host serialization, identifiers, resources, and bootstrap traces agree with React 18 and React 19'
	);
}
...
async function validateImplementedExports(reference, target) {
	const modules = {
		react: await import(
			pathToFileURL(path.join(root, 'packages', 'react-compat', 'dist', `react${target}.js`)).href
		),
		'react/jsx-runtime': await import(
			pathToFileURL(path.join(root, 'packages', 'react-compat', 'dist', `jsx-runtime${target}.js`))
				.href
		),
		'react/jsx-dev-runtime': await import(
			pathToFileURL(
				path.join(root, 'packages', 'react-compat', 'dist', `jsx-dev-runtime${target}.js`)
			).href
		),
		'react-dom': await import(
			pathToFileURL(path.join(root, 'packages', 'react-dom-compat', 'dist', `react${target}.js`))
				.href
		),
		'react-dom/client': await import(
			pathToFileURL(path.join(root, 'packages', 'react-dom-compat', 'dist', `client${target}.js`))
				.href
		)
	};
	modules['react-dom/server'] = await import(
		pathToFileURL(path.join(root, 'packages', 'react-dom-compat', 'dist', `server${target}.js`))
			.href
	);
	if (target === 19)
		modules['react-dom/static'] = await import(
			pathToFileURL(path.join(root, 'packages', 'react-dom-compat', 'dist', 'static19.js')).href
		);
	if (target === 19)
		modules['react/compiler-runtime'] = await import(
			pathToFileURL(path.join(root, 'packages', 'react-compat', 'dist', 'compiler-runtime.js')).href
		);
	const missing = [];
...
async function runExactPhase2(target) {
	const { stdout } = await execFileAsync(
		process.execPath,
		['scripts/run-exact-react-phase2.mjs', String(target)],
...
async function runExactPhase3(target) {
	const { stdout } = await execFileAsync(
		process.execPath,
		['scripts/run-exact-react-phase3.mjs', String(target)],
		{ cwd: root, encoding: 'utf8', windowsHide: true }
	);
	return JSON.parse(stdout.trim());
}
...
async function runExactPhase4(target) {
	const { stdout } = await execFileAsync(
		process.execPath,
		['scripts/run-exact-react-phase4.mjs', String(target)],
		{ cwd: root, encoding: 'utf8', windowsHide: true }
	);
	return JSON.parse(stdout.trim());
}
...
async function runExactPhase5(target) {
	const { stdout } = await execFileAsync(
		process.execPath,
		['scripts/run-exact-react-phase5.mjs', String(target)],
		{ cwd: root, encoding: 'utf8', windowsHide: true }
	);
	return JSON.parse(stdout.trim());
}
...
async function runExactPhase6(target) {
	const { stdout } = await execFileAsync(
		process.execPath,
		['scripts/run-exact-react-phase6.mjs', String(target)],
		{ cwd: root, encoding: 'utf8', windowsHide: true }
	);
	return JSON.parse(stdout.trim());
}
...
function validatePhase1Result() {
	...
	if (
		!Array.isArray(result.compatibility?.supported) ||
		!Array.isArray(result.compatibility?.deferred)
	) {
...
function validatePhase2Result() {
	...
	if (
		!Array.isArray(result.compatibility?.supported) ||
		!Array.isArray(result.compatibility?.approximate) ||
		!Array.isArray(result.compatibility?.deferred)
	) {
