...
const results = [];
const diagnostics = captureExpectedDiagnostics();
	...
	try {
		for (const target of [18, 19]) {
			const aliases = new Map([
				...
				[
					'react/jsx-runtime',
					path.join(workspace, `packages/react-compat/dist/jsx-runtime${target}.js`)
				],
				[
					'react/jsx-dev-runtime',
					path.join(workspace, `packages/react-compat/dist/jsx-dev-runtime${target}.js`)
				],
				[
					'react/compiler-runtime',
					path.join(workspace, 'packages/react-compat/dist/compiler-runtime.js')
				],
				['react-dom', path.join(workspace, `packages/react-dom-compat/dist/react${target}.js`)],
				[
					'react-dom/client',
					path.join(workspace, `packages/react-dom-compat/dist/client${target}.js`)
				]
			]);
				...
				plugins: [
					{
						...
						setup(buildApi) {
							buildApi.onResolve(
								{
									filter:
										/^react(?:\/jsx-(?:dev-)?runtime|\/compiler-runtime)?$|^react-dom(?:\/client)?$/
								},
								(args) => {
			...
			if (!inputs.some((value) => value.endsWith(`/react-compat/dist/react${target}.js`)))
				throw new Error(`React ${target} compatibility singleton was not bundled`);
			if (!inputs.some((value) => value.includes('react-reconciler')))
				throw new Error(`React ${target} genuine reconciler was not bundled`);
			if (inputs.some((value) => /node_modules\/react\/(?:index|cjs\/react)/.test(value)))
				throw new Error(
					`React ${target} genuine React implementation leaked into the compatibility bundle`
				);
			if (result.afterUnmount !== 0 || !result.effects.includes('cleanup:1'))
				throw new Error(`React ${target} reconciler cleanup failed`);
			const failedAssertions = Object.entries(result.assertions)
				.filter(([, value]) => !value)
				.map(([name]) => name);
			if (failedAssertions.length > 0)
				throw new Error(
					`React ${target} generic reconciler scenarios failed: ${failedAssertions.join(', ')} ${JSON.stringify(result)}`
				);
		...
		const r3fAliases = new Map([
			...
			[
				'react/jsx-dev-runtime',
				path.join(workspace, 'packages/react-compat/dist/jsx-dev-runtime19.js')
			],
		...
		const r3fBuild = await build({
			stdin: {
				contents:
					'export { runR3fScenario } from "./apps/react-reconciler-reference-19/src/r3f-scenario.mjs";',
				resolveDir: workspace,
		...
		if (!r3f.meshCreated || r3f.updatedPosition !== 2 || r3f.frameCount !== 1 || !r3f.disposed)
			throw new Error(`R3F compatibility scenario failed: ${JSON.stringify(r3f)}`);
		const canvasOutput = path.join(temporary, 'r3f-canvas-19.mjs');
		const canvasBuild = await build({
			stdin: {
				contents:
					'export { runCanvasScenario } from "./apps/react-reconciler-reference-19/src/canvas-scenario.mjs";',
				resolveDir: workspace,
			...
			plugins: [
				{
					name: 'exact-r3f-canvas-singleton',
					setup(buildApi) {
						buildApi.onResolve(
							{ filter: /^react(?:\/jsx-(?:dev-)?runtime)?$|^react-dom(?:\/client)?$/ },
							(args) => {
		...
		if (
			...
			!canvas.meshCreated ||
			!canvas.contextBridged ||
			canvas.size?.[0] !== 320 ||
			!canvas.disposed ||
			canvas.domAfterUnmount !== 0
		)
			throw new Error(`R3F Canvas compatibility scenario failed: ${JSON.stringify(canvas)}`);
		const breadthOutput = path.join(temporary, 'renderer-breadth-19.mjs');
		const breadthBuild = await build({
			stdin: {
				contents:
					'export { runBreadthScenario } from "./apps/react-reconciler-reference-19/src/breadth-scenario.mjs";',
				resolveDir: workspace,
			...
			plugins: [
				{
					name: 'exact-renderer-breadth-singleton',
					setup(buildApi) {
						buildApi.onResolve(
							{ filter: /^react(?:\/jsx-(?:dev-)?runtime)?$|^react-dom(?:\/client)?$/ },
							(args) => {
		...
		if (
			!breadth.ink.renderedInitial ||
			!breadth.ink.renderedUpdate ||
			!breadth.ink.effects.includes('cleanup:1') ||
			breadth.reactPdf.header !== '%PDF' ||
			breadth.reactPdf.bytes < 500
		)
			throw new Error(`Renderer breadth scenario failed: ${JSON.stringify(breadth)}`);
		...
		const r3f18Aliases = new Map([
			...
			[
				'react/jsx-dev-runtime',
				path.join(workspace, 'packages/react-compat/dist/jsx-dev-runtime18.js')
			],
		...
		const r3f18Build = await build({
			stdin: {
				contents:
					'export { runR3f18Scenario } from "./apps/react-reconciler-reference-18/src/r3f-scenario.mjs";',
				resolveDir: workspace,
				sourcefile: 'r3f-18-entry.mjs'
			},
			bundle: true,
			format: 'esm',
			platform: 'node',
		...
		if (
			!r3f18.meshCreated ||
			r3f18.updatedPosition !== 3 ||
			r3f18.frameCount !== 1 ||
			!r3f18.disposed
		)
			throw new Error(`R3F 18 compatibility scenario failed: ${JSON.stringify(r3f18)}`);
		console.log(
			JSON.stringify(
				{
					schemaVersion: 1,
					suite: 'generic-react-reconciler',
					results,
					r3f: {
						react19: { ...r3f, canvas, status: 'compatible' },
						react18: { ...r3f18, status: 'compatible' }
					},
					breadth
				},
				null,
...
function captureExpectedDiagnostics() {
	...
	return {
		...
		assert() {
			const allowed = [
				...
				/(?:Warning: )?An update to (?:Root|Scene2?|InkApp) inside a test was not wrapped in act/
			];
			...
			if (unexpected.length) {
				throw new Error(
					`React compatibility scenarios emitted unexpected diagnostics:\n${unexpected.join('\n---\n')}`
				);
			}
			for (const pattern of allowed) {
				if (!observed.some((message) => pattern.test(message))) {
					throw new Error(`React compatibility scenario no longer emitted ${pattern}`);
				}
			}
		}
	};
