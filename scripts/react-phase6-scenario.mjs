...
export async function collectReactPhase6Trace({ React, ReactDOM, ReactDOMServer, baseline }) {
	...
	try {
		...
		const resourceHints =
			baseline.startsWith('19') && typeof ReactDOM?.preload === 'function'
		...
		return {
			...
			identifiers: {
				...
				prefixed: ids.every((id) => id.includes('cert-'))
			},
			bootstrap,
			resourceHints
		};
...
function generatedHostCases(React) {
	...
	cases.push(
		...
		React.createElement(
			'svg',
			{ viewBox: '0 0 4 4', strokeWidth: 2 },
			React.createElement('path', { d: 'M0 0h4' })
		),
		React.createElement('div', {
			style: { marginTop: 4, lineHeight: 2, WebkitLineClamp: 2, msTransition: 'all' }
		}),
