...
const encodeProtocol =
	typeof reactiveRuntime.encodeReactiveProtocolValue === 'function'
		? reactiveRuntime.encodeReactiveProtocolValue
		: (value) => value;
const decodeProtocol =
	typeof reactiveRuntime.decodeReactiveProtocolValue === 'function'
...
function serverSnapshot(value) {
	...
	registerReactiveListKey(
		snapshot,
		(item) => item.id,
		'reactive benchmark server snapshot',
		'member:id'
	);
		...
		run: () =>
			timeReconcile(() => {
				...
				return {
					...
					next: clone(original),
					assert: (current) => {
						if (current.records[Math.floor(size / 2)] !== retained)
							throw new Error('Unkeyed identical refresh lost item identity');
					}
				};
			}, 1)
	},
		...
		run: () =>
			timeReconcile(() => {
				...
				return {
					...
					assert: (current) => {
						if (current.records[Math.floor(size / 2)] !== retained)
							throw new Error('Keyed identical refresh lost item identity');
					}
				};
			}, 1)
	},
		...
		run: () =>
			timeReconcile(() => {
				...
				return {
					...
					assert: (current) => {
						if (current.records[changedIndex].title !== 'Changed')
							throw new Error('One-item change was not applied');
						if (current.records[changedIndex - 1] !== retained)
							throw new Error('One-item change lost adjacent identity');
					}
				};
			}, 2)
	},
		...
		run: () =>
			timeReconcile(() => {
				...
				return {
					...
					next: serverSnapshot(next),
					assert: (current) => {
						if (!current.records[0].detail.done || current.records[1].detail.done)
							throw new Error('One-percent change was not applied precisely');
					}
				};
			}, 2)
	},
		...
		run: () =>
			timeReconcile(() => {
				...
				return {
					...
					next: serverSnapshot(next),
					assert: (current) => {
						if (current.records[1] !== first || current.records[0].id !== String(size - 1))
							throw new Error('Rotation lost keyed identity or order');
					}
				};
			}, 2)
	},
		...
		run: () =>
			timeReconcile(() => {
				...
				return {
					...
					assert: (current) => {
						if (
							current.records.length !== size ||
							current.records[0] !== retained ||
							current.records.at(-1).id !== 'new-99'
						) {
	...
	},
	{
		...
		run: () => {
			...
			if (
				...
				decoded.length !== size ||
				decoded[Math.floor(size / 2)].id !== String(Math.floor(size / 2))
			) {
