...
import {
	...
	expressions,
	rewriteModule
} from '../packages/expressions/dist/index.js';
...
const records = Array.from(
	{ length: 1_000 },
	(_, index) =>
		`const value${index} = source.items.filter(item => item.index === ${index}).map(item => item.value);`
).join('\n');
...
if (!rewritten.emit().code.includes('=== 2000'))
	throw new Error('Expression rewrite did not update the selected literal');
if (queryMs > 750)
	throw new Error(`Expression query budget exceeded: ${queryMs.toFixed(1)}ms > 750ms`);
if (rewriteMs > 500)
	throw new Error(`Expression rewrite budget exceeded: ${rewriteMs.toFixed(1)}ms > 500ms`);
if (bindingMs > 750)
	throw new Error(
		`Expression incremental binding budget exceeded: ${bindingMs.toFixed(1)}ms > 750ms`
	);
