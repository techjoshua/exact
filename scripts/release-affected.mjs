...
export async function createAffectedReleasePlan(base = process.env.RELEASE_BASE ?? "HEAD^") {
  ...
  const globalChange = changedFiles.some(filename =>
  ...
  const reactCompatibility = selected.some(workspace =>
    /react-(?:compat|compatibility|dom-compat)|react-reconciler-reference/.test(`${workspace.name} ${workspace.directory}`)
  ) || changedScripts.some(filename => /react|r3f/.test(filename));
  const r3fBrowser = selected.some(workspace =>
    /react-(?:compat|dom-compat)|react-reconciler-reference/.test(`${workspace.name} ${workspace.directory}`)
  ...
  return Object.freeze({
    ...
    packageTestDirectories: Object.freeze(selected
    ...
    apps: Object.freeze({
      ...
      serverComponents: selected.some(workspace => workspace.directory === "apps/server-components"),
      shipping: selected.some(workspace => workspace.directory === "apps/shipping-calculator")
    }),
    reactCompatibility,
    r3fBrowser,
    expressions: selected.some(workspace =>
...
function changedFilesSince(base) {
  ...
  return [...new Set(sets.flat().map(filename => filename.replaceAll("\\", "/")).filter(Boolean))].sort();
}
