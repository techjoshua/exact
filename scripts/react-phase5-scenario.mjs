...
export async function collectReactPhase5Trace({
	...
	const markupTree = React.createElement(
		'label',
		{
			htmlFor: 'field',
			tabIndex: 0,
	...
	const pipeable = await pipeableText(
		ReactDOMServer,
		React.createElement('main', null, 'streamed')
	);

	const dom = new JSDOM('<!doctype html><div id="root"><button>0</button></div>', {
	...
	try {
		await React.act(async () => {
			root = ReactDOMClient.hydrateRoot(container, React.createElement(Counter));
		});
