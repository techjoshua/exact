...
export async function collectReactPhase1Trace({ React, ReactDOMClient, JSDOM, baseline }) {
  ...
  function useCounter() {
    ...
    const [reduced, dispatch] = React.useReducer((value, action) => action === "next" ? value + 1 : value, 0);
    ...
    renders.push({ count, reduced, doubled, refStable: stableRef === ref, callbackStable: stableCallback === increment });
  ...
  function Counter() {
    ...
    return React.createElement("div", null,
      React.createElement("button", { id: "increment", onClick: state.increment, "data-double": state.doubled }, String(state.count)),
      React.createElement("button", { id: "same", onClick: () => state.dispatch("same") }, "same"),
      React.createElement("button", { id: "reduce", onClick: () => state.dispatch("next") }, String(state.reduced))
    );
  }
  ...
  try {
    await act(async () => { root.render(React.createElement(Counter)); });
    const initialHtml = normalizedHtml(container);
    await act(async () => { click(dom.window, container, "#same"); });
    const rendersAfterBailout = renders.length;
    await act(async () => { click(dom.window, container, "#increment"); });
    await act(async () => { click(dom.window, container, "#reduce"); });
    ...
    return {
      ...
      client: { initialHtml, updatedHtml, rendersAfterBailout, renders, unmountedHtml: normalizedHtml(container) }
    };
  } finally {
...
function click(window, container, selector) {
  container.querySelector(selector)?.dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
}
