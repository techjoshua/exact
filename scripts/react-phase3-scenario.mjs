...
export async function collectReactPhase3Trace({
	React,
	ReactDOM,
	ReactDOMClient,
	JSDOM,
	...
	const dom = new JSDOM('<!doctype html><div id="root"></div><aside id="portal"></aside>', {
		url: 'https://exact.invalid/'
	});
	const previousGlobals = installDomGlobals(dom.window);
	let resolveLazy;
	const lazyModule = new Promise((resolve) => {
		resolveLazy = resolve;
	});
	...
	function App() {
		...
		return React.createElement(
			...
			React.createElement(
				'button',
				{ onClick: () => start(() => setValue('second')) },
				`${value}/${deferred}/${pending}`
			),
			ReactDOM.createPortal(
				React.createElement(PortalValue),
				dom.window.document.getElementById('portal')
			),
			React.createElement(
				React.Suspense,
	...
	function PortalValue() {
		return React.createElement('span', null, React.useContext(Tone));
	}
	...
	try {
		...
		return {
			...
			transitioned,
			resolved,
			unmount: { root: container.innerHTML, portal: portal.innerHTML }
		};
	} finally {
