...
for (const fixture of cases) {
  const result = await build({
    ...
    platform: "browser",
    treeShaking: true,
    external: ["@exact/core", "@exact/request", "@exact/react-compat", "@exact/react-compat/interop"]
  });
  const source = result.outputFiles[0].text;
  for (const token of fixture.absent) {
    if (source.includes(token)) throw new Error(`${fixture.name} router bundle retained ${JSON.stringify(token)}`);
  }
