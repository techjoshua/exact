...
/** Checks source ownership and waits for every file inspection before reporting success. */
export async function checkSourceArchitecture(root, { readSource = readFile } = {}) {
	...
	for (const file of await scriptFiles(path.join(root, 'scripts'))) {
		inspectSize(file, await readSource(file, 'utf8'), /\.(?:test|spec)\.[cm]?[jt]sx?$/.test(file));
	}
	...
	async function inspectLargeNonTypeScriptDomains() {
		...
		for (const file of files) {
			...
			const lines = (await readSource(file, 'utf8')).split(/\r?\n/).length;
	...
	function inspectSize(file, source, isTest = false) {
		...
		if (logicalLines > limit) {
			...
			if (ceiling === undefined)
				violations.push(
					`${relative}: ${logicalLines} logical lines exceeds the ${limit}-line limit`
				);
	...
	async function declaredPublicSubpaths(packageRoot) {
		try {
			const manifest = JSON.parse(await readSource(path.join(packageRoot, 'package.json'), 'utf8'));
