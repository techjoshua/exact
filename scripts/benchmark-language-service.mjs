...
fs.writeFileSync(benchmarkConfig, JSON.stringify({
  compilerOptions: {
    target: "ES2022",
    module: "NodeNext",
...

{
  ...
  try {
    ...
    console.log(`language-service warm implementation-only (${samples} samples): median ${percentile(timings, 0.5).toFixed(2)}ms, p95 ${percentile(timings, 0.95).toFixed(2)}ms`);
  } finally {
    service.dispose();
  }
}

{
  ...
  try {
    service.synchronize([
      { kind: "upsert", filename: model, source: "export interface Model { value: number }\nexport const model: Model = { value: 1 };" },
      { kind: "upsert", filename: consumer, source: 'import { model } from "./LanguageBenchmarkModel.js";\nexport const value: number = model.value;' }
    ]);
    const result = synchronize(service, [{
      kind: "upsert",
      filename: model,
      source: 'export interface Model { value: string }\nexport const model: Model = { value: "changed" };'
    }]);
    assert(result.update.affectedFiles.some(filename => filename.toLowerCase() === consumer.replaceAll("\\", "/").toLowerCase()));
    assert(result.update.diagnostics.some(diagnostic => diagnostic.code === "TS2322"));
    console.log(`language-service exported-shape propagation: ${result.elapsed.toFixed(2)}ms, ${result.update.affectedFiles.length} affected, TS2322 confirmed`);
  } finally {
    service.dispose();
  }
}
...
// Minimal public TypeScript builder host retained as a correctness reference.
{
  ...
  const sources = new Map([
    [model, { version: 1, source: "export interface Model { value: number }\nexport const model: Model = { value: 1 };" }],
    [consumer, { version: 1, source: 'import { model } from "./LanguageReferenceModel.js";\nexport const value: number = model.value;' }]
  ]);
  const options = {
  ...
  sources.set(model, { version: 2, source: 'export interface Model { value: string }\nexport const model: Model = { value: "changed" };' });
