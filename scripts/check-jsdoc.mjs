...
for (const relative of contractModules) {
  ...
  for (const statement of sourceFile.statements) {
    ...
    if (ts.isFunctionDeclaration(statement)) {
      ...
      if (!documentedOverloads.has(name)) report(relative, sourceFile, statement, `exported function ${name}`);
    } else if (ts.isClassDeclaration(statement)) {
      const className = statement.name?.text ?? "default";
      if (!hasJsDoc(statement)) report(relative, sourceFile, statement, `exported class ${className}`);
      for (const member of statement.members) {
        if (!ts.isMethodDeclaration(member) || hasModifier(member, ts.SyntaxKind.PrivateKeyword)) continue;
        const name = member.name.getText(sourceFile);
        if (!hasJsDoc(member)) report(relative, sourceFile, member, `public method ${className}.${name}`);
      }
    }
  }
}
