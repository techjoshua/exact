...
function printSummary(elapsedMs) {
  ...
  process.stdout.write(`${"total".padEnd(width)}  ${(elapsedMs / 1000).toFixed(1).padStart(7)}s  ${failure ? "failed" : "passed"}\n`);
}
...
async function runAffected() {
  ...
  if (plan.packageTestDirectories.length) {
    verification.push(task("affected package tests", [
      "exec", "--", "vitest", "run",
      ...plan.packageTestDirectories,
      "--testTimeout=15000"
    ]));
  }
  if (plan.apps.serverComponents) verification.push(task("server-component tests", ["run", "test:server-components"]));
